...
# 前端提交的 CheckMacValue
FRONTEND_CHECKMACVALUE = "078E562DD03A770D49212408C786E02E1A6DFA54D357FC6FA6F92C05801FF776"
...
def compare_with_frontend():
    ...
    calculated_mac = calculate_checkmacvalue_step_by_step(EXACT_FRONTEND_PARAMS, HASH_KEY, HASH_IV)
    
    print(f"\n📊 CheckMacValue 比較結果:")
    print(f"   前端提交值: {FRONTEND_CHECKMACVALUE}")
    print(f"   我們計算值: {calculated_mac}")
    print(f"   是否一致: {'✅ 完全匹配！' if calculated_mac == FRONTEND_CHECKMACVALUE else '❌ 不匹配'}")
    
    if calculated_mac == FRONTEND_CHECKMACVALUE:
        print(f"\n🎉 成功！CheckMacValue 計算完全正確")
        print(f"   這意味著我們的計算邏輯與 ECPay 規範完全一致")
        print(f"   問題可能出現在其他地方（例如網路、ECPay 測試環境等）")
    else:
        print(f"\n🚨 CheckMacValue 不匹配")
        ...
        for i, (c1, c2) in enumerate(zip(FRONTEND_CHECKMACVALUE, calculated_mac)):
            ...
            print(f"   長度差異: 前端={len(FRONTEND_CHECKMACVALUE)}, 計算={len(calculated_mac)}")
