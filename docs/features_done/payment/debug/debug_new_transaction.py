...
def generate_checkmacvalue_with_time_variations():
    ...
    for test_time in test_times:
        ...
        encoded_raw_string = urllib.parse.quote_plus(
            raw_string, encoding="utf-8"
        ).lower()
        calculated_mac = (
        ...
        if test_time.endswith(":00") or test_time.endswith(":30"):
            print(
                f"   {test_time}: {calculated_mac[:16]}... {'✅' if calculated_mac == target_mac else '❌'}"
            )
...
def try_different_field_combinations():
    ...
    for cf1 in custom_field1_options:
        for cf3 in custom_field3_options:
            ...
            print(
                f"   CF1='{cf1}', CF3='{cf3}': {calculated_mac[:16]}... {'✅' if calculated_mac == target_mac else '❌'}"
            )
