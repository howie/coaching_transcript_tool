...
def generate_checkmacvalue_exact(params, hash_key, hash_iv):
    ...
    for key, value in sorted_items:
        ...
        # 顯示需要編碼的特殊值
        if str_value != encoded_value or '教練' in str_value or '#' in str_value or 'http' in str_value:
    ...
    # SHA256 計算
    calculated_hash = hashlib.sha256(encoded_raw_string.encode('utf-8')).hexdigest().upper()
...
def compare_with_backend_output():
    ...
    backend_checkmacvalue = "AA7D90BC9146ED1FC65CD9EC368FF1001673D1C6812F6FCCA467A6AA45DAFE95"
    
    # 我們重新計算的值
    calculated_mac, details = generate_checkmacvalue_exact(EXACT_BACKEND_PARAMS, HASH_KEY, HASH_IV)
    
    print(f"\n📊 CheckMacValue 比較:")
    print(f"   後端實際計算: {backend_checkmacvalue}")
    print(f"   我們重新計算: {calculated_mac}")
    print(f"   是否一致: {'✅ 是' if calculated_mac == backend_checkmacvalue else '❌ 否'}")
...
""")
    ...
    print(f"\n📋 期望的後端參數值:")
    for key in ['MerchantTradeDate', 'CustomField1', 'CustomField3', 'MerchantMemberID', 'MerchantTradeNo']:
