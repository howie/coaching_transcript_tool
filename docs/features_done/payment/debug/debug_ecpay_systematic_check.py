    ...
    for field in numeric_fields:
        if field in params:
            ...
            if not isinstance(value, str):
                issues_found.append(
                    f"❌ {field} 應為字串格式，目前是 {type(value).__name__}"
                )
            else:
...
def generate_frontend_comparison():
    ...
    for field in critical_fields:
        if field in params:
            print(
                f"   {field}: '{params[field] if field != 'CheckMacValue' else calculation['checkmacvalue']}'"
            )
...
if __name__ == "__main__":
    ...
    print(
        "   前端目標值: 5DCC55EEA22463E1F51DDF39CD65D0FB21D085A2595849F6B2D5997EAE0D3216"
    )
    print(
        f"   是否匹配: {'✅ 是' if calculation_result['checkmacvalue'] == '5DCC55EEA22463E1F51DDF39CD65D0FB21D085A2595849F6B2D5997EAE0D3216' else '❌ 否'}"
    )

    if (
        calculation_result["checkmacvalue"]
        != "5DCC55EEA22463E1F51DDF39CD65D0FB21D085A2595849F6B2D5997EAE0D3216"
    ):
        print("\n💡 下一步建議:")
        print("1. 請在前端加入以下 debug 代碼輸出完整參數")
        print("2. 確認 MerchantTradeDate 的精確時間")
        print("3. 逐個比較每個參數值的完全一致性")
