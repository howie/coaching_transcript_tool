...
async def format_root():
    """格式化 API 根端點"""
    return {"message": "Coaching Transcript Tool Format API v2.2 - CF Workers"}
...
async def format_transcript_endpoint(
    ...
    try:
        # CF Workers 檔案大小限制 (10MB)
        if file.size and file.size > 10 * 1024 * 1024:  # 10MB limit for CF Workers
            raise HTTPException(
                status_code=413,
                detail="File too large. Maximum size is 10MB on CF Workers."
            )
        ...
        logger.info(f"Successfully processed file '{file.filename}' -> '{output_filename}'")
        ...
        raise HTTPException(
            status_code=400, 
            detail=f"Unrecognized file format: {e}"
        )
