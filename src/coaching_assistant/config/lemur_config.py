...
class LeMURConfigLoader:
    ...
    def load_config(self, force_reload: bool = False) -> LeMURConfig:
        ...
        try:
            ...
            model_settings = ModelSettings(
                default_model=model_config.get('default_model', "claude_sonnet_4_20250514"),
            ...
            performance_settings = PerformanceSettings(
                ...
                large_transcript_threshold=adaptive_config.get('large_transcript_threshold', 15000),
                medium_transcript_threshold=adaptive_config.get('medium_transcript_threshold', 8000),
                max_concurrent_batches=concurrent_config.get('max_concurrent_batches', 3),
                enable_parallel_processing=concurrent_config.get('enable_parallel_processing', True)
            )
            
            # Load prompts
            prompts = LeMURPrompts(
                speaker_identification=yaml_config.get('speaker_identification', {}),
                punctuation_optimization=yaml_config.get('punctuation_optimization', {}),
            ...
            logger.info(f"   Prompt languages: {list(self._config.prompts.speaker_identification.keys())}")
    ...
    def _apply_env_overrides(self, model_settings: ModelSettings) -> ModelSettings:
        ...
        if hasattr(settings, 'LEMUR_MAX_OUTPUT_SIZE') and settings.LEMUR_MAX_OUTPUT_SIZE:
            model_settings.max_output_size = int(settings.LEMUR_MAX_OUTPUT_SIZE)
            logger.info(f"🔧 Max output size override: {settings.LEMUR_MAX_OUTPUT_SIZE}")
            
        if hasattr(settings, 'LEMUR_COMBINED_MODE') and settings.LEMUR_COMBINED_MODE is not None:
...
def get_speaker_prompt(language: str = "chinese", variant: str = "default", context: Optional[Dict[str, Any]] = None) -> Optional[str]:
    """Get speaker identification prompt."""
    return get_prompt_template("speaker", language, variant, context)


def get_punctuation_prompt(language: str = "chinese", variant: str = "default", context: Optional[Dict[str, Any]] = None) -> Optional[str]:
    """Get punctuation optimization prompt."""
    return get_prompt_template("punctuation", language, variant, context)


def get_combined_prompt(language: str = "chinese", variant: str = "default", context: Optional[Dict[str, Any]] = None) -> Optional[str]:
