...
def process_subscription_maintenance(self):
    ...
    try:
        ...
        if self.retry_num < self.max_retries:
            logger.info(f"🔄 Retrying subscription maintenance (attempt {self.retry_num + 1})")
...
def process_failed_payment_retry(self, payment_id: str):
    ...
    try:
        ...
        if success:
            ...
            db.commit()
            logger.warning(f"❌ Payment retry failed for {payment_id} (attempt {payment.retry_count})")
            
            return {"status": "retry_failed", "payment_id": payment_id, "retry_count": payment.retry_count}
        
    except Exception as e:
        logger.error(f"💥 Payment retry task failed for {payment_id}: {e}")
        
        if self.retry_num < self.max_retries:
            logger.info(f"🔄 Retrying payment retry task (attempt {self.retry_num + 1})")
...
def send_payment_failure_notifications(self, notification_data: dict):
    ...
    try:
        logger.info(f"📧 Sending payment failure notification: {notification_data['notification_type']}")
...
def cleanup_old_webhook_logs():
    ...
    try:
        ...
        deleted_count = db.query(WebhookLog).filter(
            WebhookLog.received_at < cutoff_date
        ).delete()
...
def _generate_maintenance_stats(db: Session) -> dict:
    ...
    try:
        ...
        success_rate = (recent_success / total_recent * 100) if total_recent > 0 else 100.0
...
def _simulate_payment_retry(payment: SubscriptionPayment, subscription: SaasSubscription) -> bool:
    """
