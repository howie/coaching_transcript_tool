    ...
    try:
        ...
        with get_db_session() as db_session:
            ...
            if recipient_emails:
                ...
                logger.info(f"✅ Daily report sent to {len(recipient_emails)} recipients")
            else:
                logger.warning("⚠️ No recipient emails configured, report generated but not sent")
...
def schedule_weekly_summary_report(self, week_start_date_str: Optional[str] = None):
    ...
    try:
        ...
        if week_start_date_str:
            week_start = datetime.fromisoformat(week_start_date_str).replace(tzinfo=timezone.utc)
        else:
        ...
        with get_db_session() as db_session:
            ...
            html_content = _format_weekly_report_email(weekly_summary, week_start, week_end)
...
def _send_email_report(
    ...
    try:
        ...
        logger.info(f"📧 Daily report email sent successfully to {len(recipients)} recipients")
...
def _aggregate_weekly_data(daily_reports: List[DailyReportData]) -> dict:
    ...
    for daily in daily_reports:
        weekly_summary["total_users_end"] = max(weekly_summary["total_users_end"], daily.total_users)
    ...
    # Calculate averages
    weekly_summary["avg_error_rate"] = sum(weekly_summary["error_rates"]) / len(weekly_summary["error_rates"]) if weekly_summary["error_rates"] else 0
    
    return weekly_summary


def _format_weekly_report_email(weekly_data: dict, week_start: datetime, week_end: datetime) -> str:
    """Format weekly summary report as HTML email."""
    
    success_rate = ((weekly_data["completed_sessions_week"] / weekly_data["total_sessions_week"]) * 100) if weekly_data["total_sessions_week"] > 0 else 0
...
def _send_weekly_report_email(
    ...
    try:
        msg = MimeMultipart()
        msg['Subject'] = f"Weekly Admin Report - {week_start.strftime('%Y-%m-%d')} to {week_end.strftime('%Y-%m-%d')} 📅"
        msg['From'] = smtp_user
        msg['To'] = ", ".join(recipients)
