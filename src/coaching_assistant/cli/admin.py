...
def grant_role(email: str, role: str, reason: str, force: bool):
    ...
    with get_db_session() as db:
        ...
        if not super_admin:
            ...
            if role == "super_admin":
                click.echo(
                    "⚠️  No super administrator found. Creating first super admin..."
                )
                ...
                click.echo(
                    "❌ No super administrator found in system. Create one first."
                )
                sys.exit(1)
    ...
    with get_db_session() as db:
        ...
        try:
            updated_user = permission_service.revoke_role(
                user_id=str(user.id), granted_by=super_admin, reason=reason
            )
            click.echo(
                f"✅ Role revoked for {email}. Now has role: {updated_user.role.value}"
            )
        except Exception as e:
