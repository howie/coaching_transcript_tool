...
class AdminDailyReportService:
    ...
    def generate_daily_report(self, target_date: Optional[datetime] = None) -> DailyReportData:
        """
        ...
        try:
            ...
            self.logger.info(f"✅ Daily report generated successfully for {report_start.date()}")
            return report_data
    ...
    def _get_user_metrics(self, start: datetime, end: datetime) -> Dict[str, Any]:
        ...
        # New users in period
        new_users_query = self.db.query(User).filter(
        ...
        active_users_count = self.db.query(
            func.count(func.distinct(TranscriptSession.user_id))
        ).filter(
            and_(TranscriptSession.created_at >= start, TranscriptSession.created_at < end)
        ).scalar() or 0
    ...
    def _get_session_metrics(self, start: datetime, end: datetime) -> Dict[str, Any]:
        ...
        provider_counts = self.db.query(
            ...
            and_(TranscriptSession.created_at >= start, TranscriptSession.created_at < end)
        ).group_by(TranscriptSession.stt_provider).all()
        
        sessions_by_provider = {provider or "unknown": count for provider, count in provider_counts}
        ...
        total_seconds = usage_stats[0] if usage_stats and usage_stats[0] is not None else 0
        total_minutes_processed = Decimal(str(total_seconds)) / 60 if total_seconds else Decimal('0')
        total_cost_raw = usage_stats[1] if usage_stats and usage_stats[1] is not None else None
        total_cost_usd = Decimal(str(total_cost_raw)) if total_cost_raw is not None else Decimal('0')
    ...
    def _get_admin_metrics(self, start: datetime, end: datetime) -> Dict[str, Any]:
        ...
        # All admin users
        admin_users_query = self.db.query(User).filter(
        ...
        admin_users = [
            {
                ...
                "admin_access_expires": user.admin_access_expires.isoformat() if user.admin_access_expires else None,
                "created_at": user.created_at.isoformat()
            }
        ...
        # Staff logins today
        staff_logins_today = self.db.query(User).filter(
            and_(
                User.role.in_([UserRole.ADMIN, UserRole.STAFF, UserRole.SUPER_ADMIN]),
    ...
    def _get_billing_metrics(self, start: datetime, end: datetime) -> Dict[str, Any]:
        ...
        try:
            subscription_counts = self.db.query(
                SaasSubscription.plan_name,
                func.count(SaasSubscription.id)
            ).filter(
                SaasSubscription.status == 'active'
            ).group_by(SaasSubscription.plan_name).all()
            
            active_subscriptions = {plan or "unknown": count for plan, count in subscription_counts}
        except Exception as e:
            self.logger.warning(f"Failed to query subscription counts (table may not exist): {e}")
            active_subscriptions = {}
        
        # Subscription changes in period (new, cancelled, upgraded)
        try:
            subscription_changes = self.db.query(SaasSubscription).filter(
                or_(
                    and_(SaasSubscription.created_at >= start, SaasSubscription.created_at < end),
                    and_(SaasSubscription.updated_at >= start, SaasSubscription.updated_at < end)
                )
            ).all()
            ...
            changes_summary = [
                {
                    ...
                    "amount": float(sub.amount_twd) / 100 if sub.amount_twd else 0.0,  # Convert from cents
                    "billing_cycle": sub.billing_cycle
                }
                for sub in subscription_changes[:20]  # Limit to recent changes
            ]
        except Exception as e:
            self.logger.warning(f"Failed to query subscription changes (table may not exist): {e}")
            changes_summary = []
    ...
    def _get_system_health_metrics(self, start: datetime, end: datetime) -> Dict[str, Any]:
        """Get system health and performance metrics."""
        self.logger.debug("📋 Collecting system health metrics...")
        
        # Calculate error rate
        total_sessions = self.db.query(TranscriptSession).filter(
            and_(TranscriptSession.created_at >= start, TranscriptSession.created_at < end)
        ).count()
        
        failed_sessions = self.db.query(TranscriptSession).filter(
            and_(
                TranscriptSession.created_at >= start,
                TranscriptSession.created_at < end,
                TranscriptSession.status == SessionStatus.FAILED
        ...
        completed_durations = self.db.query(
            func.avg(TranscriptSession.duration_seconds)
        ).filter(
            and_(
                TranscriptSession.created_at >= start,
                TranscriptSession.created_at < end,
    ...
    def _get_activity_metrics(self, start: datetime, end: datetime) -> Dict[str, Any]:
        ...
        top_users_query = self.db.query(
            ...
            TranscriptSession, User.id == TranscriptSession.user_id
        ).filter(
            and_(TranscriptSession.created_at >= start, TranscriptSession.created_at < end)
        ).group_by(
            User.id
        ...
        top_users = [
            {
                ...
                "success_rate": (completed_sessions / session_count * 100) if session_count > 0 else 0.0
            }
    ...
    def format_report_for_email(self, report: DailyReportData) -> str:
        ...
        success_rate = (report.completed_sessions / report.total_sessions * 100) if report.total_sessions > 0 else 0
    ...
    def _format_plan_distribution(self, users_by_plan: Dict[str, int], total_users: int) -> str:
        """Format plan distribution as HTML table rows."""
        rows = []
        for plan, count in users_by_plan.items():
            percentage = (count / total_users * 100) if total_users > 0 else 0
            rows.append(f"<tr><td>{plan.upper()}</td><td>{count}</td><td>{percentage:.1f}%</td></tr>")
        return ''.join(rows)
    ...
    def _format_provider_list(self, sessions_by_provider: Dict[str, int]) -> str:
        ...
        for provider, count in sessions_by_provider.items():
            items.append(f"<li><strong>{provider.title()}:</strong> {count} sessions</li>")
        return ''.join(items)
    ...
    def _format_admin_users_list(self, admin_users: List[Dict]) -> str:
        ...
        for user in admin_users:
            last_login = "Never" if not user['last_admin_login'] else datetime.fromisoformat(user['last_admin_login'].replace('Z', '+00:00')).strftime('%Y-%m-%d')
            items.append(f"<p><strong>{user['role'].upper()}:</strong> {user['email']} (Last login: {last_login})</p>")
        return ''.join(items)
    ...
    def _format_subscription_list(self, active_subscriptions: Dict[str, int]) -> str:
        ...
        for plan, count in active_subscriptions.items():
            items.append(f"<li><strong>{plan.upper()}:</strong> {count} active subscriptions</li>")
        return ''.join(items)
