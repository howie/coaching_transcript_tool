...
from sqlalchemy import Column, String, Float, Integer, JSON, Boolean, Enum as SQLEnum
...
class PlanConfiguration(Base, TimestampMixin):
    ...
    # Plan identification
    plan_type = Column(SQLEnum(UserPlan), unique=True, nullable=False, index=True)
...
class SubscriptionHistory(Base, TimestampMixin):
    ...
    # Plan change details
    old_plan = Column(SQLEnum(UserPlan), nullable=True)
    new_plan = Column(SQLEnum(UserPlan), nullable=False)
