...
class BillingAnalytics(BaseModel):
    ...
    user_segment = Column(String(20), nullable=True)  # 'new', 'growing', 'power', 'enterprise'
    ...
    __table_args__ = (
        UniqueConstraint('user_id', 'period_type', 'period_start', name='uq_billing_analytics_period'),
    ...
    def gross_margin_usd(self) -> float:
        ...
        return float(self.total_revenue_usd) - float(self.total_provider_cost_usd)
    ...
    def revenue_per_minute(self) -> float:
        ...
        if self.total_minutes_processed > 0:
            return float(self.total_revenue_usd) / float(self.total_minutes_processed)
        return 0.0
    ...
    def cost_per_minute(self) -> float:
        ...
        if self.total_minutes_processed > 0:
            return float(self.total_provider_cost_usd) / float(self.total_minutes_processed)
        return 0.0
    ...
    def is_power_user(self) -> bool:
        ...
        return (
            float(self.plan_utilization_percentage) > 70 and
            self.days_active_in_period > (self._get_period_days() * 0.6) and
            float(self.avg_sessions_per_active_day) > 3
        )
    ...
    def is_at_risk(self) -> bool:
        ...
        return (
            float(self.churn_risk_score) > 0.7 or
            self.days_active_in_period < (self._get_period_days() * 0.2) or
            float(self.plan_utilization_percentage) < 10
        )
    ...
    def calculate_customer_health_score(self) -> float:
        ...
        utilization_score = min(float(self.plan_utilization_percentage), 100) * 0.3
        activity_score = (self.days_active_in_period / self._get_period_days()) * 100 * 0.3
        usage_score = min(float(self.avg_sessions_per_active_day) * 10, 100) * 0.2
        success_score = float(self.success_rate_percentage) * 0.2
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "plan_info": {
                ...
                "change_date": self.plan_change_date.isoformat() if self.plan_change_date else None
            },
            "revenue": {
                "total": float(self.total_revenue_usd),
                "subscription": float(self.subscription_revenue_usd),
                "overage": float(self.usage_overage_usd),
                "one_time": float(self.one_time_fees_usd),
                "per_minute": self.revenue_per_minute
            },
            "usage": {
                "sessions_created": self.sessions_created,
                "transcriptions_completed": self.transcriptions_completed,
                "total_minutes": float(self.total_minutes_processed),
                "total_hours": self.total_hours_processed,
                "active_days": self.days_active_in_period,
                "avg_sessions_per_day": float(self.avg_sessions_per_active_day),
                "avg_session_duration": self.avg_session_duration_minutes
            },
            "billing": {
                "original_transcriptions": self.original_transcriptions,
                "free_retries": self.free_retries,
                "paid_retranscriptions": self.paid_retranscriptions,
                "overage_minutes": float(self.overage_minutes)
            },
            "costs": {
                "google_stt": float(self.google_stt_cost_usd),
                "assemblyai": float(self.assemblyai_cost_usd),
                "total_provider": float(self.total_provider_cost_usd),
                "per_minute": self.cost_per_minute
            },
            "metrics": {
                "plan_utilization": float(self.plan_utilization_percentage),
                "churn_risk_score": float(self.churn_risk_score),
                "success_rate": float(self.success_rate_percentage),
            ...
            "predictions": {
                "next_month_usage": float(self.predicted_next_month_usage),
                "upgrade_probability": float(self.upgrade_probability),
                "lifetime_value": float(self.lifetime_value_estimate)
            },
            "exports": {
                "by_format": self.exports_by_format,
                "total": self.total_exports
            },
            "user_profile": {
                "signup_date": self.user_signup_date.isoformat() if self.user_signup_date else None,
                "tenure_days": self.user_tenure_days,
            ...
            "quality": {
                "avg_processing_time": float(self.avg_processing_time_seconds),
                "support_tickets": self.support_tickets_count,
    ...
    def create_from_usage_data(
        ...
        return cls(
            ...
            # Revenue
            total_revenue_usd=revenue_data.get('total_revenue', 0),
            subscription_revenue_usd=revenue_data.get('subscription_revenue', 0),
            usage_overage_usd=revenue_data.get('overage_revenue', 0),
            one_time_fees_usd=revenue_data.get('one_time_fees', 0),
            
            # Usage
            sessions_created=usage_data.get('sessions_created', 0),
            transcriptions_completed=usage_data.get('transcriptions_completed', 0),
            total_minutes_processed=usage_data.get('total_minutes_processed', 0),
            ...
            overage_minutes=usage_data.get('overage_minutes', 0),
            
            # Provider costs
            google_stt_cost_usd=usage_data.get('google_stt_cost', 0),
            assemblyai_cost_usd=usage_data.get('assemblyai_cost', 0),
            total_provider_cost_usd=usage_data.get('total_provider_cost', 0),
            
            # Metrics
            plan_utilization_percentage=usage_data.get('plan_utilization', 0),
            days_active_in_period=usage_data.get('days_active', 0),
            avg_sessions_per_active_day=usage_data.get('avg_sessions_per_day', 0),
            churn_risk_score=usage_data.get('churn_risk_score', 0),
            ...
            # Performance
            avg_processing_time_seconds=usage_data.get('avg_processing_time', 0),
            success_rate_percentage=usage_data.get('success_rate', 100),
            support_tickets_count=usage_data.get('support_tickets', 0),
            
            # Predictions
            predicted_next_month_usage=usage_data.get('predicted_usage', 0),
            upgrade_probability=usage_data.get('upgrade_probability', 0),
            lifetime_value_estimate=usage_data.get('ltv_estimate', 0),
