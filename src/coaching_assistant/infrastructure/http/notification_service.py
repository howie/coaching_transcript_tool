...
class EmailNotificationService(NotificationService):
    ...
    async def send_payment_failure_notification(self, user_email: str, payment_details: Dict[str, Any]) -> bool:
        ...
        if not self.enabled:
            logger.warning("📧 Email service not configured, logging notification instead")
            logger.info(f"📧 [MOCK EMAIL] Payment failure for {user_email}: {payment_details}")
            return True
    ...
    async def send_payment_retry_notification(self, user_email: str, retry_details: Dict[str, Any]) -> bool:
        ...
        if not self.enabled:
            logger.warning("📧 Email service not configured, logging notification instead")
            logger.info(f"📧 [MOCK EMAIL] Payment retry for {user_email}: {retry_details}")
            return True
    ...
    async def send_subscription_cancellation_notification(self, user_email: str, cancellation_details: Dict[str, Any]) -> bool:
        ...
        if not self.enabled:
            logger.warning("📧 Email service not configured, logging notification instead")
            logger.info(f"📧 [MOCK EMAIL] Subscription cancelled for {user_email}: {cancellation_details}")
            return True
    ...
    async def send_plan_downgrade_notification(self, user_email: str, downgrade_details: Dict[str, Any]) -> bool:
        ...
        if not self.enabled:
            logger.warning("📧 Email service not configured, logging notification instead")
            logger.info(f"📧 [MOCK EMAIL] Plan downgraded for {user_email}: {downgrade_details}")
            return True
