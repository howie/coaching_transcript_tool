...
class InMemoryUsageLogRepository(UsageLogRepoPort):
    ...
    def get_usage_summary(
        ...
        billable_logs = [log for log in logs if log.is_billable]
        total_cost = sum(log.cost_usd for log in billable_logs if log.cost_usd) or Decimal('0.00')
        
        # Transcription type breakdown
        type_breakdown = {}
        for transcription_type in TranscriptionType:
            count = len([log for log in logs if log.transcription_type == transcription_type])
        ...
        for log in logs:
            if log.stt_provider:
                provider_breakdown[log.stt_provider] = provider_breakdown.get(log.stt_provider, 0) + 1
