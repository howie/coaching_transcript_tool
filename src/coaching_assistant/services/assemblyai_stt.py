...
class AssemblyAIProvider(STTProvider):
    ...
    def _upload_audio(self, audio_uri: str) -> str:
        ...
        if audio_uri.startswith('gs://'):
            ...
            try:
                ...
                logger.info(f"✅ Generated signed URL for AssemblyAI: {signed_url[:60]}...")
                return signed_url
    ...
    def _poll_transcription_status(self, transcript_id: str, progress_callback=None) -> Dict[str, Any]:
        ...
        logger.info(f"Polling transcription status for ID: {transcript_id} (every {polling_interval}s)")
            ...
            try:
                ...
                if status == 'completed':
                    if progress_callback:
                        ...
                        progress_callback(100, "AssemblyAI transcription completed", elapsed_minutes)
                    return result
                elif status == 'error':
                ...
                if progress_callback:  # Update progress on every poll (now every 30 seconds)
                    elapsed_minutes = (time.time() - start_time) / 60.0
    ...
    def _parse_transcript_result(
            ...
            if 'words' in result and result['words']:
                ...
                for word in result['words']:
                    if current_start is None:
                        ...
                        current_speaker = self._convert_speaker_id(word.get('speaker', 0))
                ...
                if text and (result.get('language_code') == 'zh' or 'zh' in (result.get('language_code') or '')):
                    text = self._process_chinese_text(text, needs_conversion)
        ...
        provider_metadata = {
            ...
            "post_processing_applied": ["remove_spaces", "convert_to_traditional"] if needs_conversion else ["remove_spaces"],
    ...
    def transcribe(
        ...
        except (STTProviderError, STTProviderUnavailableError, 
                STTProviderQuotaExceededError, STTProviderInvalidAudioError):
