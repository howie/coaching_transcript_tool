...
class UsageAnalyticsService:
    ...
    def record_usage_snapshot(self, user_id: UUID, period_type: str = "hourly") -> UsageHistory:
        ...
        existing = self.db.query(UsageHistory).filter(
            and_(
        ...
        if existing:
            ...
            self.db.commit()
            logger.info(f"✅ Updated existing {period_type} snapshot for user {user_id}")
            return existing
        else:
            # Create new snapshot
    ...
    def get_usage_trends(
        ...
        logger.info(f"📈 Getting usage trends for user {user_id}, period: {period}, group_by: {group_by}")
        ...
        history_data = self.db.query(UsageHistory).filter(
            and_(
                UsageHistory.user_id == user_id,
        ...
        ).order_by(UsageHistory.period_start).all()
        
        # If no history data, fall back to usage_log aggregation
        if not history_data:
            return self._aggregate_usage_logs_for_trends(user_id, start_date, end_date, group_by)
    ...
    def predict_usage(self, user_id: UUID) -> Dict[str, Any]:
        ...
        early_avg = sum(d["sessions"] for d in early_period) / len(early_period) if early_period else 0
        late_avg = sum(d["sessions"] for d in late_period) / len(late_period) if late_period else 0
        ...
        if avg_daily_minutes > 0:
            ...
            if days_to_limit <= 30:  # Within next month
                limit_date = (datetime.utcnow() + timedelta(days=days_to_limit)).isoformat()
        ...
        prediction = {
            ...
            "current_trend": "growing" if growth_rate > 5 else "stable" if growth_rate > -5 else "declining"
        }
        
        logger.info(f"✅ Generated predictions for user {user_id}: {predicted_sessions} sessions, {predicted_minutes} minutes")
        return prediction
    ...
    def _calculate_period_boundaries(self, timestamp: datetime, period_type: str) -> Tuple[datetime, datetime]:
    ...
    def _aggregate_usage_data(self, user_id: UUID, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
        ...
        sessions = self.db.query(TranscriptionSession).filter(
            and_(
        ...
        free_retries = len([log for log in usage_logs if log.transcription_type == TranscriptionType.RETRY_FAILED])
        
        # Provider breakdown
        google_minutes = sum(log.duration_minutes for log in usage_logs if log.stt_provider == "google")
        assemblyai_minutes = sum(log.duration_minutes for log in usage_logs if log.stt_provider == "assemblyai")
        ...
        if completed_logs:
            processing_times = [
                (log.transcription_completed_at - log.transcription_started_at).total_seconds()
        ...
        return {
            ...
            "unique_clients": len(set(log.client_id for log in usage_logs if log.client_id)),
    ...
    def _analyze_usage_patterns(self, usage_data: List[Dict[str, Any]], user: User) -> Dict[str, Any]:
        ...
        plan_config = self.plan_limits.get_plan_limit(user.plan.value)
        utilization = (total_minutes / plan_config.max_minutes) * 100 if plan_config.max_minutes > 0 else 0
        ...
        if mid_point > 0:
            ...
            late_avg = sum(d["sessions"] for d in usage_data[mid_point:]) / (len(usage_data) - mid_point)
            growth_rate = ((late_avg - early_avg) / early_avg * 100) if early_avg > 0 else 0
        else:
            growth_rate = 0
