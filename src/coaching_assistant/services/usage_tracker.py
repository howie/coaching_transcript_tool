...
class UsageTracker:
    ...
    def increment_usage(
        ...
        logger.info(f"Incremented {metric} for user {user_id}: {current_value} -> {new_value}")
        ...
        summary = {
            ...
            "sessions": {
                ...
                "percentage": calculate_percentage(
                    usage["session_count"],
                    limits.max_sessions_per_month
                ),
                "remaining": max(0, limits.max_sessions_per_month - usage["session_count"])
                    if limits.max_sessions_per_month != -1 else -1
            },
            "transcriptions": {
                ...
                "remaining": max(0, limits.max_transcriptions_per_month - usage["transcription_count"])
                    if limits.max_transcriptions_per_month != -1 else -1
            },
            "minutes": {
                ...
                "remaining": max(0, limits.max_minutes_per_month - usage["usage_minutes"])
                    if limits.max_minutes_per_month != -1 else -1
            },
    ...
    def record_usage_history(
        ...
        # For now, just log it
        logger.info(f"Usage history: user={user_id}, action={action}, metadata={metadata}")
