...
class EnglishSmoothingConfig(BaseSmoothingConfig):
    ...
    filler_whitelist: List[str] = field(default_factory=lambda: [
        "um", "uh", "er", "ah", "yeah", "okay", "right", "well",
        "you know", "like", "so"
    ])
...
class EnglishProcessor(LanguageProcessor):
    ...
    def detect_language(self, utterances: List[Utterance]) -> bool:
        ...
        english_words = {
            ...
            'what', 'when', 'where', 'why', 'how', 'who', 'which', 'so', 'about'
        }
        total_words = 0
    ...
    def determine_punctuation(self, text: str) -> str:
        ...
        # Question indicators
        question_words = ['what', 'where', 'when', 'why', 'how', 'who', 'which', 'do', 'does', 'did', 'can', 'could', 'should', 'would']
        ...
        # Exclamation indicators
        if any(word in text_lower for word in ['wow', 'amazing', 'incredible', 'fantastic']):
            return '!'
...
class SpeakerBoundarySmoother:
    ...
    def __init__(self, config: BaseSmoothingConfig, language_processor: LanguageProcessor):
        self.config = config
    ...
    def _try_merge_segments(self, current: Utterance, next_utt: Utterance) -> Optional[Tuple[Utterance, Optional[Utterance]]]:
    ...
    def _check_short_head_backfill(self, current: Utterance, next_utt: Utterance) -> Optional[Tuple[Utterance, Optional[Utterance]]]:
        ...
        if (head_duration < self.config.th_short_head_sec and 
            not self.language_processor.has_terminal_punctuation(
    ...
    def _check_filler_backfill(self, current: Utterance, next_utt: Utterance) -> Optional[Tuple[Utterance, Optional[Utterance]]]:
        ...
        if (first_word.text in filler_words and 
            word_duration < th_filler_max_sec):
            
            return self._merge_words_to_previous(current, next_utt, [first_word])
            
        return None
    
    def _check_echo_backfill(self, current: Utterance, next_utt: Utterance) -> Optional[Tuple[Utterance, Optional[Utterance]]]:
    ...
    def _get_words_within_duration(self, words: List[WordTimestamp], max_duration: float) -> List[WordTimestamp]:
    ...
    def _find_echo_in_previous(self, previous_words: List[WordTimestamp], quoted_content: str) -> bool:
    ...
    def _calculate_jaccard_similarity(self, words1: List[WordTimestamp], words2: List[WordTimestamp]) -> float:
    ...
    def _find_echo_words(self, previous_words: List[WordTimestamp], current_words: List[WordTimestamp]) -> List[WordTimestamp]:
...
class PunctuationRepairer:
    ...
    def __init__(self, config: BasePunctuationConfig, language_processor: LanguageProcessor):
        self.config = config
        self.language_processor = language_processor
    ...
    def repair_punctuation(self, utterances: List[Utterance]) -> List[ProcessedSegment]:
        ...
        logger.info(f"Punctuation repair completed. Created {len(segments)} segments from {len(utterances)} utterances")
        return segments
    
    def _flatten_words_with_speaker(self, utterances: List[Utterance]) -> List[WordWithSpeaker]:
    ...
    def _create_segment_with_punctuation(self, words: List[WordWithSpeaker]) -> ProcessedSegment:
        ...
        text_with_punct = self.language_processor.normalize_punctuation(text + punctuation)
...
class TranscriptSmoothingService:
    ...
    def __init__(self, language: Optional[str] = None, config: Optional[BaseProcessorConfig] = None):
    ...
    def smooth_and_punctuate(
        ...
        **kwargs
        ...
        if language == "auto":
            detected_language = LanguageProcessorFactory.detect_language(transcript_input.utterances)
        else:
            try:
                detected_language = SupportedLanguage(language.lower())
            except ValueError:
                logger.warning(f"Unsupported language '{language}', falling back to auto-detection")
                detected_language = LanguageProcessorFactory.detect_language(transcript_input.utterances)
        
        # Create language processor
        language_processor = LanguageProcessorFactory.create_processor(detected_language)
        ...
        # Create processors
        boundary_smoother = SpeakerBoundarySmoother(config.smoothing, language_processor)
        punctuation_repairer = PunctuationRepairer(config.punctuation, language_processor)
        
        logger.info(f"Starting transcript smoothing for language: {detected_language.value}")
        
        # Process
        smoothed_utterances = boundary_smoother.smooth_boundaries(transcript_input.utterances)
        final_segments = punctuation_repairer.repair_punctuation(smoothed_utterances)
        
        # Calculate statistics
        original_word_count = sum(len(utt.words) for utt in transcript_input.utterances)
        moved_word_count = boundary_smoother.stats.short_first_segment + boundary_smoother.stats.filler_words
