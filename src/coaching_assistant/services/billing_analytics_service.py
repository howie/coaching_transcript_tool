...
from sqlalchemy import and_, func, desc, asc, case, extract
...
class BillingAnalyticsService:
    ...
    def get_admin_overview(
        ...
        billing_data = self.db.query(BillingAnalytics).filter(
            and_(
    ...
    def get_revenue_trends(
        ...
        logger.info(f"📈 Getting revenue trends for {months} months, period: {period_type}")
    ...
    def get_customer_segmentation(
        ...
        if include_predictions:
            ...
            for segment in segments:
                segment["predictions"] = self._get_segment_predictions(segment["segment"])
    ...
    def get_user_analytics_detail(
        ...
        historical_data = self.db.query(BillingAnalytics).filter(
            and_(
                BillingAnalytics.user_id == user_id,
                BillingAnalytics.period_start >= start_date
            )
        ).order_by(BillingAnalytics.period_start).all()
        
        # Calculate metrics
        total_revenue = sum(float(record.total_revenue_usd) for record in historical_data)
        tenure_days = (datetime.utcnow() - user.created_at).days if user.created_at else 0
    ...
    def get_cohort_analysis(
        ...
        logger.info(f"📊 Getting cohort analysis: {cohort_type}, size: {cohort_size}, metric: {metric}")
        ...
        ).group_by(date_trunc).order_by(date_trunc.desc()).limit(cohort_size).all()
    ...
    def get_churn_analysis(
        ...
        analysis = {
            "summary": summary,
            "at_risk_users": [self._format_churn_user(user) for user in at_risk_users[:50]],
    ...
    def get_plan_performance_analysis(
        ...
        for plan in ["FREE", "PRO", "ENTERPRISE"]:
            plan_data = self.db.query(BillingAnalytics).filter(
                and_(
            ...
            if plan_data:
                plan_performance[plan] = {
                    ...
                    "total_revenue": sum(float(record.total_revenue_usd) for record in plan_data),
                    "avg_utilization": sum(float(record.plan_utilization_percentage) for record in plan_data) / len(plan_data),
                    "avg_health_score": sum(record.calculate_customer_health_score() for record in plan_data) / len(plan_data),
                    "churn_risk_users": len([r for r in plan_data if float(r.churn_risk_score) > 0.7])
                }
            ...
            for record in billing_data:
                ...
                if user:
                    data.update({
                        "user_email": user.email,
                        "user_name": user.name,
                        "user_signup_date": user.created_at.isoformat() if user.created_at else None
                    })
    ...
    def refresh_user_analytics(
        ...
        existing = self.db.query(BillingAnalytics).filter(
            and_(
                BillingAnalytics.user_id == user_id,
                BillingAnalytics.period_type == period_type,
                BillingAnalytics.period_start == period_start
        ...
        if existing and not force_rebuild:
            ...
            updated_data = self._calculate_user_billing_data(user_id, period_start, period_end)
            ...
            billing_data = self._calculate_user_billing_data(user_id, period_start, period_end)
    ...
    def refresh_all_analytics(
        ...
        for user in users:
            try:
                result = self.refresh_user_analytics(user.id, period_type, force_rebuild)
    ...
    def get_health_score_distribution(self) -> Dict[str, Any]:
        ...
        recent_data = self.db.query(BillingAnalytics).filter(
        ...
        health_scores = [record.calculate_customer_health_score() for record in recent_data]
    ...
    def _calculate_revenue_metrics(self, billing_data: List[BillingAnalytics]) -> Dict[str, float]:
        ...
        total_cost = sum(float(record.total_provider_cost_usd) for record in billing_data)
        total_minutes = sum(float(record.total_minutes_processed) for record in billing_data)
        
        return {
            "total_revenue": total_revenue,
            "subscription_revenue": sum(float(record.subscription_revenue_usd) for record in billing_data),
            "overage_revenue": sum(float(record.usage_overage_usd) for record in billing_data),
            "one_time_fees": sum(float(record.one_time_fees_usd) for record in billing_data),
            ...
            "avg_revenue_per_minute": total_revenue / total_minutes if total_minutes > 0 else 0
        }
    
    def _calculate_usage_metrics(self, billing_data: List[BillingAnalytics]) -> Dict[str, Any]:
        ...
        total_transcriptions = sum(record.transcriptions_completed for record in billing_data)
        total_minutes = sum(float(record.total_minutes_processed) for record in billing_data)
        success_rates = [float(record.success_rate_percentage) for record in billing_data if record.success_rate_percentage]
        ...
        return {
            ...
            "avg_session_duration": total_minutes / total_transcriptions if total_transcriptions > 0 else 0,
    ...
    def _get_customer_segments(self, billing_data: List[BillingAnalytics]) -> List[Dict[str, Any]]:
        ...
        for record in billing_data:
            ...
            segments[segment]["total_utilization"] += float(record.plan_utilization_percentage)
        ...
        for segment_data in segments.values():
            if segment_data["user_count"] > 0:
                segment_data["avg_revenue_per_user"] = segment_data["total_revenue"] / segment_data["user_count"]
                segment_data["avg_utilization"] = segment_data["total_utilization"] / segment_data["user_count"]
        
        return list(segments.values())
    
    def _get_top_users(self, billing_data: List[BillingAnalytics], limit: int = 10) -> List[Dict[str, Any]]:
        """Get top users by revenue."""
        # Sort by revenue and get top users
        sorted_data = sorted(billing_data, key=lambda x: float(x.total_revenue_usd), reverse=True)
        ...
        for record in sorted_data[:limit]:
            ...
            if user:
                top_users.append({
                    ...
                    "total_minutes_processed": float(record.total_minutes_processed),
                    "plan_utilization_percentage": float(record.plan_utilization_percentage),
    ...
    def _get_trend_data(
        ...
        while current_date < period_end:
            ...
            period_data = self.db.query(BillingAnalytics).filter(
                and_(
            ...
            trends.append({
                ...
                "revenue": sum(float(record.total_revenue_usd) for record in period_data),
                "users": len(set(record.user_id for record in period_data)),
                "sessions": sum(record.sessions_created for record in period_data),
                "minutes": sum(float(record.total_minutes_processed) for record in period_data),
    ...
    def _determine_user_segment(self, user_id: UUID) -> str:
        ...
        recent_usage = self.db.query(UsageHistory).filter(
            and_(
        ...
        avg_minutes = sum(float(record.audio_minutes_processed) for record in recent_usage) / len(recent_usage)
        avg_utilization = sum(record.utilization_percentage for record in recent_usage) / len(recent_usage)
    ...
    def _get_cohort_metric_data(self, cohort_period: datetime, metric: str) -> List[Dict[str, Any]]:
        """Get metric data for a specific cohort over time."""
        # Simplified implementation
        return [{"period": "month_1", "value": 100}, {"period": "month_2", "value": 85}]
    
    def _analyze_customer_segments(self, billing_data: List[BillingAnalytics]) -> List[Dict[str, Any]]:
    ...
    def _calculate_lifetime_value(self, user_id: UUID) -> float:
        ...
        historical_revenue = self.db.query(
            func.sum(BillingAnalytics.total_revenue_usd)
        ).filter(BillingAnalytics.user_id == user_id).scalar() or 0
    ...
    def _get_churn_predictions(self, at_risk_users: List[BillingAnalytics]) -> Dict[str, Any]:
        """Get churn predictions for at-risk users."""
        return {
            "predicted_churn_30_days": len(at_risk_users) * 0.3,
            "predicted_revenue_loss": sum(float(u.total_revenue_usd) for u in at_risk_users) * 0.3
        }
    
    def _analyze_upgrade_patterns(self, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
    ...
    def _generate_plan_recommendations(self, plan_performance: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    def _generate_plan_forecasts(self, plan_performance: Dict[str, Any]) -> Dict[str, Any]:
