...
class PlanLimits:
    ...
    def get_plan_limit(self, plan_name) -> PlanLimit:
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"⚠️ Failed to get plan from database for {plan_name}: {e}"
                )
                logger.info(f"🔄 Falling back to hardcoded limits for {plan_name}")
                # Fallback to hardcoded limits
                return self._FALLBACK_LIMITS.get(
                    plan_enum, self._FALLBACK_LIMITS[PlanName.FREE]
                )
    ...
    def get_upgrade_suggestion(self, current_plan: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                if upgrade_suggestion:
                    logger.info(
                        f"📋 Retrieved upgrade suggestion for {current_plan} from database"
                    )
                    return upgrade_suggestion
            except Exception as e:
                logger.warning(f"⚠️ Failed to get upgrade suggestion from database: {e}")
                logger.info("🔄 Using fallback upgrade suggestions")
    ...
    @staticmethod
    def format_limit_message(limit_type: str, current: int, limit: int) -> str:
        ...
        if remaining == 0:
            ...
            return (
                f"You have used {current} out of {limit} {limit_type} ({percentage}%)"
            )
        else:
