...
class PlanConfigurationService:
    ...
    def _get_from_db(self, plan_type: UserPlan) -> Optional[PlanConfiguration]:
        ...
        db = self.get_db()
        try:
            config = db.query(PlanConfiguration).filter(
    ...
    def _convert_db_to_dict(self, db_config: PlanConfiguration) -> Dict[str, Any]:
        ...
        return {
            ...
            "pricing": {
                "monthly_twd": db_config.monthly_price_twd_cents / 100 if db_config.monthly_price_twd_cents else 0,
                "annual_twd": db_config.annual_price_twd_cents / 100 if db_config.annual_price_twd_cents else 0,
                "monthly_usd": db_config.monthly_price_cents / 100 if db_config.monthly_price_cents else 0,
                "annual_usd": db_config.annual_price_cents / 100 if db_config.annual_price_cents else 0,
    ...
    def _normalize_limits(self, limits: Dict[str, Any]) -> Dict[str, Any]:
        ...
        key_mappings = {
            ...
            "max_total_minutes": ["max_total_minutes", "maxTotalMinutes", "maxMinutes", "max_minutes"],
            "max_transcription_count": ["max_transcription_count", "maxTranscriptionCount", "maxTranscriptions"],
            "max_file_size_mb": ["max_file_size_mb", "maxFileSizeMb", "maxFileSize"],
            "export_formats": ["export_formats", "exportFormats"],
            "concurrent_processing": ["concurrent_processing", "concurrentProcessing", "concurrentJobs"],
    ...
    def get_all_plans(self, include_inactive: bool = False) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            plans = []
            for plan_type in [UserPlan.FREE, UserPlan.STUDENT, UserPlan.PRO, UserPlan.COACHING_SCHOOL]:
                if plan_type in self.FALLBACK_CONFIGS:
    ...
    def validate_usage(self, plan_type: UserPlan, action: str, current_usage: Dict[str, Any], 
                      action_params: Dict[str, Any] = None) -> Tuple[bool, str, Dict[str, Any]]:
        """
        ...
        try:
            ...
            if action == "check_minutes":
                ...
                if max_minutes == -1:  # Unlimited
                    return True, "Minutes usage allowed (unlimited)", {"type": "minutes", "limit": -1}
                ...
                if projected_usage > max_minutes:
                    return False, f"Monthly minutes limit exceeded ({max_minutes} minutes)", {
                        "type": "minutes",
                ...
                return True, "Minutes usage allowed", {
                    "type": "minutes",
                    "current": current_minutes,
                    "limit": max_minutes,
                ...
                if file_size > max_size:
                    return False, f"File size exceeds limit ({max_size}MB)", {
                        "type": "file_size",
                        "current": file_size,
                        "limit": max_size
                    }
                ...
                if export_format not in available_formats:
                    return False, f"Export format '{export_format}' not available", {
                        "type": "export_format",
                        "requested": export_format,
                        "available": available_formats
                    }
                
                return True, "Export format allowed", {
                    "type": "export_format",
                    "requested": export_format,
                    "available": available_formats
                }
            ...
            # Phase 2: Sessions and transcriptions are unlimited
                ...
                return True, f"{action} allowed (unlimited in Phase 2)", {"type": action, "limit": -1}
            
            else:
                logger.warning(f"Unknown action for validation: {action}")
                return True, "Action allowed (unknown action)", {"type": "unknown"}
                
        except Exception as e:
            logger.error(f"Error validating usage for {plan_type}, action {action}: {e}")
            # Fail open - allow action on error
            return True, "Validation temporarily unavailable", {"type": "error"}

    def get_upgrade_suggestion(self, current_plan: UserPlan) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if suggested_minutes == -1:
                ...
                ratio = suggested_minutes / current_minutes if current_minutes > 0 else 0
                if ratio >= 2:
                    benefits.append(f"{int(ratio)}x 音檔分鐘數 ({suggested_minutes} 分鐘/月)")
                else:
            ...
            return {
                ...
                "comparison_url": f"/billing/compare?from={current_plan.value}&to={suggested_plan.value}"
            }
