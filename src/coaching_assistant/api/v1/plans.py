...
# Hardcoded plan configurations removed - now using repository pattern
# through use cases
    ...
    try:
        ...
        for plan_data in plans_data:
            ...
            plan_response = PlanResponse(
                ...
                pricing=PlanPricing(
                    monthly_twd=int(
                        plan_data["pricing"]["monthly_twd"] * 100
                    ),  # Convert to cents
                ...
                limits=PlanLimits(
                    max_total_minutes=(
                        plan_data["limits"]["maxTotalMinutes"]
                        if plan_data["limits"]["maxTotalMinutes"] != "unlimited"
                        else -1
                    ),
        ...
        logger.info(
            f"Retrieved {len(converted_plans)} plans for user {current_user.id}"
        )
...
async def get_current_plan_status(
    ...
    try:
        ...
        usage_status = {
            ...
            "planDisplayName": plan_info.get(
                "display_name", _get_plan_value(current_user.plan)
            ),
            "currentUsage": {
            ...
            "approachingLimits": {
                "sessions": (
                    len(
                        [
                            w
                            for w in usage_validation["warnings"]
                            if w["type"] == "session_count_warning"
                        ]
                    )
                    > 0
                ),
            ...
            "nextReset": current_usage.get(
                "period_start"
            ),  # Use period info from use case
        }
                ...
                if suggested_config:
                    usage_status["upgradeSuggestion"] = {
                        ...
                        "keyBenefits": _get_upgrade_benefits(
                            current_user.plan,
                            getattr(UserPlan, suggested_plan_id),
                        ),
                        "pricing": suggested_config["pricing"],
                        "tagline": suggested_config.get(
        ...
        subscription_info = {
            ...
            "endDate": None,
            "active": (
                _get_plan_value(current_user.plan) != UserPlan.FREE.value
            ),  # More accurate active status
            "stripeSubscriptionId": None,
        }

        # Get actual subscription data if user has paid plan with graceful
        # error handling
        if _get_plan_value(current_user.plan) != UserPlan.FREE.value:
            try:
                subscription_data = (
                    subscription_retrieval_use_case.get_current_subscription(
                        current_user.id
                    )
                )
                ...
                if subscription_data.get("subscription"):
                    ...
                    logger.warning(
                        f"User {current_user.id} has plan "
                        f"{_get_plan_value(current_user.plan)} but no active subscription"
                    )
            except DomainException as e:
                logger.warning(
                    f"Subscription service returned domain error for user "
                    f"{current_user.id}: {e}"
                )
                subscription_info["active"] = False
                # Continue with basic plan info - subscription error is not
                # critical
            except Exception as e:
                logger.error(
                    f"Critical error retrieving subscription for user "
                    f"{current_user.id}: {e}"
                )
                subscription_info["active"] = False
                # Continue with basic plan info - subscription error should not
                # break plans endpoint
        ...
        logger.error(
            f"Failed to get current plan status for user {current_user.id}: {e}"
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def compare_plans(
    ...
    try:
        ...
        logger.debug(
            f"Retrieved comparison data with "
            f"{comparison_data.get('total_plans', 0)} plans"
        )
        ...
        if current_plan_enum != "ENTERPRISE":
            ...
            recommended_upgrade = {
                ...
                "pricing": {
                    "monthlyUsd": (29.99 if suggested_plan_name == "PRO" else 99.99)
                },
...
async def get_plan_by_id(
    ...
    try:
        ...
        plans_response = await get_available_plans_v1(
            current_user, plan_retrieval_use_case
        )
    ...
    try:
            ...
            legacy_format = {
                ...
                "limits": {
                    ...
                    "maxTotalMinutes": (
                        -1
                        if plan_data["limits"]["maxTotalMinutes"] == "unlimited"
                        else plan_data["limits"]["maxTotalMinutes"]
                    ),
                    "maxTranscriptionCount": -1,  # Unlimited in new system
                    "maxFileSizeMb": plan_data["limits"]["maxFileSizeMb"],
                    "exportFormats": (
                        plan_data["limits"].get("exportFormats", ["json", "txt"])
                    ),
                    "concurrentProcessing": (
                        plan_data["limits"].get("concurrentProcessing", 1)
                    ),
                ...
                "display": {
                    "isPopular": (
                        plan_data.get("sort_order") == 2
                    ),  # Assume middle plan is popular
                    "colorScheme": "blue",
        ...
        return {
            ...
            "featuresComparison": {
                ...
                "concurrentProcessing": (
                    "Number of files you can transcribe simultaneously"
                ),
                "prioritySupport": (
...
async def validate_action(
    ...
    try:
        ...
        if (
            ...
            and current_plan_value != UserPlan.ENTERPRISE.value
        ):
