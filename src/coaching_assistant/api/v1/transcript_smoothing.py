...
def resolve_transcript_session_id(
    session_id: str, user_id: str, db: DBSession
) -> tuple[str, bool]:
    """
...
class ProcessedSegment(BaseModel):
    ...
    source_utterance_indices: list[int] = Field(
        ..., description="Source utterance indices"
    )
...
class ProcessingStats(BaseModel):
    ...
    moved_word_count: int = Field(
        ..., description="Number of words moved between speakers"
    )
...
class TranscriptSmoothingResponse(BaseModel):
    ...
    segments: list[ProcessedSegment] = Field(
        ..., description="Processed transcript segments"
    )
...
class LeMURSegment(BaseModel):
    ...
    speaker: str = Field(
        ...,
        description="Corrected speaker identifier (教練/客戶 or Coach/Client)",
    )
    start_ms: int = Field(..., description="Start time in milliseconds")
    end_ms: int = Field(..., description="End time in milliseconds")
...
class LeMURSmoothingResponse(BaseModel):
    ...
    segments: list[LeMURSegment] = Field(
        ..., description="LeMUR-processed transcript segments"
    )
...
class DBProcessingRequest(BaseModel):
    ...
    custom_prompts: Optional[Dict[str, str]] = Field(
        None, description="Optional custom prompts"
    )
...
async def lemur_smooth_endpoint(
    request: TranscriptSmoothingRequest,
    current_user=Depends(get_current_user_dependency),
) -> LeMURSmoothingResponse:
    """Call the actual LeMUR smoothing function."""
    return await lemur_smooth_transcript(request, current_user)
...
async def lemur_speaker_identification_endpoint(
    request: TranscriptSmoothingRequest,
    current_user=Depends(get_current_user_dependency),
) -> LeMURSmoothingResponse:
    """Call the actual LeMUR speaker identification function."""
    return await lemur_speaker_identification(request, current_user)
...
async def lemur_combined_processing(
    ...
    try:
        ...
        logger.error(
            f"LeMUR combined processing failed for user {current_user.email}: {e}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=500,
...
async def lemur_speaker_identification(
    ...
    try:
        ...
        logger.error(
            f"LeMUR speaker identification failed for user {current_user.email}: {e}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=500,
...
async def lemur_punctuation_optimization(
    ...
    try:
        ...
        logger.error(
            f"LeMUR punctuation optimization failed for user {current_user.email}: {e}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=500,
...
async def lemur_smooth_transcript(
    ...
    try:
        ...
        if session_language == "auto":
            ...
            session_language = detected_lang
            logger.info(
                f"🌍 LANGUAGE AUTO-DETECTED: {original_language} -> {session_language}"
            )
        elif session_language == "chinese":
            session_language = "zh-TW"
            logger.info(
                f"🌍 LANGUAGE MAPPING: {original_language} -> {session_language} (Traditional Chinese)"
            )
        elif session_language == "english":
            session_language = "en-US"
            logger.info(
                f"🌍 LANGUAGE MAPPING: {original_language} -> {session_language}"
            )
        else:
        ...
        sample_text = " ".join(
            [seg.get("text", "")[:50] for seg in lemur_segments[:3]]
        )[:200]
        logger.info(f"📝 SAMPLE INPUT TEXT: {sample_text}...")

        # Check if Traditional Chinese conversion will be needed
        is_chinese = session_language.startswith("zh")
        if is_chinese:
            logger.info(
                "🔄 TRADITIONAL CHINESE CONVERSION: Will be applied to final output"
            )
        ...
        logger.info("=" * 80)
        logger.info("✅ LEMUR TRANSCRIPT SMOOTHING COMPLETED SUCCESSFULLY!")
        logger.info("=" * 80)
        logger.info(f"👤 USER: {current_user.email}")
        logger.info(f"🌍 LANGUAGE: {session_language}")
        logger.info(f"📊 SEGMENTS: {len(response_segments)}")
        logger.info(f"⏱️ PROCESSING TIME: {processing_time_ms}ms")
        logger.info(f"🎭 SPEAKER MAPPING: {smoothed_result.speaker_mapping}")
        logger.info(f"🔧 IMPROVEMENTS: {smoothed_result.improvements_made}")
        if is_chinese:
            logger.info("🔄 TRADITIONAL CHINESE: ✅ Applied")
        logger.info("=" * 80)
        ...
        logger.error(
            f"LeMUR transcript smoothing failed for user {current_user.email}: {e}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=500,
...
async def lemur_speaker_identification_from_db(
    ...
    try:
        ...
        db_segments = (
            ...
            .filter(TranscriptSegmentModel.session_id == transcript_session_id)
        ...
        for segment in db_segments:
            ...
            speaker_label = (
                f"Speaker_{segment.speaker_id}" if segment.speaker_id else "A"
            )
        ...
        if segment_updates > 0:
            db.commit()
            logger.info(
                f"Updated {segment_updates} segments with corrected speaker assignments"
            )
        else:
        ...
        return LeMURSmoothingResponse(
            ...
            speaker_mapping=smoothed_result.speaker_mapping,
            improvements_made=smoothed_result.improvements_made
            + [f"Updated {segment_updates} speaker assignments in database"],
        ...
        logger.error(
            f"DB-based speaker identification failed for session {session_id}: {e}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=500,
...
async def lemur_punctuation_optimization_from_db(
    ...
    try:
        ...
        return LeMURSmoothingResponse(
            ...
            speaker_mapping=smoothed_result.speaker_mapping,
            improvements_made=smoothed_result.improvements_made
            + [f"Updated {segment_updates} text segments in database"],
            processing_notes=f"DB-based processing: {smoothed_result.processing_notes}",
            processing_time_ms=processing_time_ms,
            success=True,
        )
        ...
        logger.error(
            f"DB-based punctuation optimization failed for session {session_id}: {e}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=500,
...
async def get_raw_assemblyai_data(
    ...
    try:
        ...
        if (
            session.stt_provider != "assemblyai"
            or not session.provider_metadata
            or "raw_assemblyai_response" not in session.provider_metadata
        ):
            raise HTTPException(
                status_code=400,
                detail={
