...
async def get_usage_history(
    ...
    logger.info(f"📊 User {current_user.id} requesting {months} months of usage history")
    
    try:
        tracking_service = UsageTrackingService(db)
        return tracking_service.get_user_usage_history(str(current_user.id), months)
    except Exception as e:
...
async def get_current_month_usage(
    ...
    usage_percentage = (
        ...
        if plan_limits["minutes_per_month"] != float("inf") else 0
    )
    ...
    return {
        ...
        "current_month": {
            ...
            "month_start": current_user.current_month_start.isoformat() if current_user.current_month_start else None
        },
        "plan_limits": plan_limits,
        "usage_percentage": round(usage_percentage, 2),
        "minutes_remaining": (
            plan_limits["minutes_per_month"] - current_user.usage_minutes
            if plan_limits["minutes_per_month"] != float("inf") else float("inf")
        ),
...
async def get_admin_usage_analytics(
    ...
    plan_filter: Optional[str] = Query(None, description="Filter by plan (free, pro, enterprise)"),
...
async def get_monthly_usage_report(
    ...
    total_transcriptions = sum(a.transcriptions_completed for a in monthly_analytics)
    total_minutes = sum(float(a.total_minutes_processed) for a in monthly_analytics)
    ...
    # Provider breakdown
    total_google_minutes = sum(float(a.google_stt_minutes) for a in monthly_analytics)
    total_assemblyai_minutes = sum(float(a.assemblyai_minutes) for a in monthly_analytics)
    ...
    return {
        ...
        "summary": {
            ...
            "avg_minutes_per_user": total_minutes / total_users if total_users > 0 else 0,
        ...
        "provider_breakdown": {
            "google": {
                ...
                "percentage": (total_google_minutes / total_minutes * 100) if total_minutes > 0 else 0
            },
            "assemblyai": {
                "minutes": total_assemblyai_minutes,
                "percentage": (total_assemblyai_minutes / total_minutes * 100) if total_minutes > 0 else 0
            }
        },
        "period": {
            "start": f"{month_year}-01",
            "end": monthly_analytics[0].period_end.isoformat() if monthly_analytics[0].period_end else None
        }
    }