...
router = APIRouter(prefix="/api/v1/admin/billing-analytics", tags=["admin", "billing-analytics"])
...
async def get_billing_analytics_overview(
    ...
    return AdminAnalyticsOverviewResponse(
        ...
        top_users=[
            BillingAnalyticsResponse(**user) 
            for user in overview["top_users"]
        ],
        trend_data=[
            TrendDataPoint(**point) 
            for point in overview["trend_data"]
        ]
    )
...
async def get_churn_analysis(
    ...
    return {
        ...
        "predictions": churn_analysis.get("predictions") if include_predictions else None
    }
