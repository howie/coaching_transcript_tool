...
async def get_current_plan_status(
    ...
    usage_status = {
        ...
        "usagePercentages": {
            "sessions": calc_percentage(current_user.session_count, plan_config["limits"]["maxSessions"]),
            "minutes": calc_percentage(current_user.usage_minutes, max_minutes),
            "transcriptions": calc_percentage(current_user.transcription_count, plan_config["limits"]["maxTranscriptionCount"])
        },
        "approachingLimits": {
            "sessions": (calc_percentage(current_user.session_count, plan_config["limits"]["maxSessions"]) or 0) >= 80,
            "minutes": (calc_percentage(current_user.usage_minutes, max_minutes) or 0) >= 80,
            "transcriptions": (calc_percentage(current_user.transcription_count, plan_config["limits"]["maxTranscriptionCount"]) or 0) >= 80
        },
        "nextReset": None
    }
    ...
    if any(usage_status["approachingLimits"].values()) and current_user.plan != UserPlan.ENTERPRISE:
        suggested_plan = UserPlan.PRO if current_user.plan == UserPlan.FREE else UserPlan.ENTERPRISE
...
async def compare_plans(
    ...
    for plan in all_plans:
        if plan["planName"] == current_user.plan.value.lower().replace("enterprise", "business"):
            plan["isCurrent"] = True
        else:
...
async def validate_action(
    ...
    if action == "create_session":
        ...
        if max_sessions != "unlimited" and max_sessions != -1 and current_user.session_count >= max_sessions:
        ...
        if max_transcriptions != "unlimited" and max_transcriptions != -1 and current_user.transcription_count >= max_transcriptions:
            validation_result.update({
                "allowed": False,
    ...
    if not validation_result["allowed"] and current_user.plan != UserPlan.ENTERPRISE:
        suggested_plan = UserPlan.PRO if current_user.plan == UserPlan.FREE else UserPlan.ENTERPRISE
        suggested_config = PLAN_CONFIGS[suggested_plan]
