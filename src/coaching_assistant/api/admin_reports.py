...
from ..tasks.admin_report_tasks import generate_and_send_daily_report, schedule_weekly_summary_report
...
class ReportRequest(BaseModel):
    ...
    send_email: bool = Field(
        True,
        description="Whether to send email report or just generate data"
    )
...
async def send_weekly_report(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to queue weekly report task: {str(e)}"
        )
...
async def get_admin_users(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to get admin users: {str(e)}"
        )
...
async def get_email_settings(
    ...
    config_status = {
        ...
        "sender_email": os.getenv("SENDER_EMAIL", os.getenv("SMTP_USER", "not-configured")),
        "admin_emails_configured": bool(os.getenv("ADMIN_REPORT_EMAILS")),
        "admin_emails_count": len(os.getenv("ADMIN_REPORT_EMAILS", "").split(",")) if os.getenv("ADMIN_REPORT_EMAILS") else 0
    }
...
async def send_test_email(
    ...
    try:
        # Create test message
        msg = MimeText("This is a test email from the Admin Reports system. ✅", 'plain', 'utf-8')
