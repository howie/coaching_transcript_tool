...
async def get_dashboard_summary(
    ...
    total_minutes_result = (
        db.query(func.coalesce(func.sum(CoachingSession.duration_min), 0))
    ...
    revenue_results = (
        db.query(
            ...
            func.coalesce(func.sum(CoachingSession.fee_amount), 0).label(
                "total_amount"
            ),
        )
        .filter(
        ...
        .group_by(CoachingSession.fee_currency)
        .all()
    )
    ...
    unique_clients_total = (
        db.query(func.count(func.distinct(CoachingSession.client_id)))
        .filter(CoachingSession.user_id == current_user.id)
        .scalar()
    )
