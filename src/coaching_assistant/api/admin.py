...
from .dependencies import require_super_admin, require_admin, get_current_user_with_permissions
...
async def revoke_user_role(
    ...
    # Get client IP for audit log
    client_ip = current_request.client.host if current_request and current_request.client else None
...
async def get_role_audit_log(
    ...
    for log in audit_logs:
        ...
        result.append(RoleAuditLogResponse(
            id=log.id,
            user_id=log.user_id,
...
async def refresh_admin_session(
    ...
    return {
        ...
        "expires_at": current_user.admin_access_expires.isoformat() if current_user.admin_access_expires else None
    }