...
def upgrade() -> None:
    ...
    op.alter_column('coaching_session', 'audio_timeseq_id', new_column_name='transcription_session_id')
    ...
    # Note: PostgreSQL automatically updates foreign key constraints when columns are renamed
...
def downgrade() -> None:
    ...
    op.add_column('coaching_session', 
        sa.Column('transcript_timeseq_id', postgresql.UUID(as_uuid=True), nullable=True)
    )
    
    op.alter_column('coaching_session', 'transcription_session_id', new_column_name='audio_timeseq_id')
