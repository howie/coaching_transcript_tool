...
coach_languages = Table(
    'coach_languages',
    BaseModel.metadata,
    Column('coach_profile_id', UUID(as_uuid=True), ForeignKey('coach_profile.id'), primary_key=True),
...
class CoachProfile(BaseModel):
    ...
    # Language settings (stored as JSON array for simplicity, or use association table)
    ...
    coach_experience = Column(String(50))  # Using String instead of Enum for flexibility
...
class CoachingPlan(BaseModel):
    ...
    coach_profile_id = Column(UUID(as_uuid=True), ForeignKey('coach_profile.id'), nullable=False)
    ...
    # Additional settings
    booking_notice_hours = Column(Integer, default=24)  # Hours notice required for booking
    ...
    def total_duration_minutes(self):
        ...
        if self.duration_minutes and self.number_of_sessions and self.number_of_sessions > 0:
