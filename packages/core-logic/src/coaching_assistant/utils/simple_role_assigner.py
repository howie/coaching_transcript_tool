...
class SimpleRoleAssigner:
    ...
    def __init__(self, client_speaking_threshold: float = 0.6):
        ...
        logger.debug(f"SimpleRoleAssigner initialized with threshold: {client_speaking_threshold}")
    ...
    def assign_roles(self, segments: List) -> Tuple[Dict[int, str], Dict[str, float]]:
        ...
        if len(speaker_metrics) == 1:
            ...
            logger.info(f"Only one speaker detected (ID: {speaker_id}), cannot assign roles")
        ...
        if len(speaker_metrics) == 2:
            ...
            logger.info(f"Multi-speaker session: {len(speaker_metrics)} speakers detected")
...
def assign_roles_simple(segments: List, threshold: float = 0.6) -> Tuple[Dict[int, str], Dict[str, float]]:
    """
