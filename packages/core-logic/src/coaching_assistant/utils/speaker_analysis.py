...
class SpeakerAnalyzer:
    ...
    def _is_question(self, text: str) -> bool:
        ...
        patterns = self.QUESTION_PATTERNS.get(self.language, self.QUESTION_PATTERNS['en'])
    ...
    def _is_coaching_question(self, text: str) -> bool:
        ...
        patterns = self.COACHING_PATTERNS.get(self.language, self.COACHING_PATTERNS['en'])
    ...
    def analyze_speakers(self, segments: List[TranscriptSegment]) -> Dict[int, SpeakerStats]:
    ...
    def assign_roles(self, segments: List[TranscriptSegment]) -> Dict[int, str]:
        ...
        if len(speaker_stats) == 1:
            ...
            logger.info(f"Single speaker detected, assigning speaker {speaker_id} as coach")
            ...
            logger.info(f"Coach scores: Speaker {speaker1_id} = {coach_score_1:.3f}, Speaker {speaker2_id} = {coach_score_2:.3f}")
            ...
            speaker_items.sort(key=lambda x: self._calculate_coach_score(x[1]), reverse=True)
    ...
    def get_confidence_metrics(self, segments: List[TranscriptSegment], roles: Dict[int, str]) -> Dict[str, float]:
        """
        ...
        avg_length_diff = client_stats.avg_segment_length - coach_stats.avg_segment_length
