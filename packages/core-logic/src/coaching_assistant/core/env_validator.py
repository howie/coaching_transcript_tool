...
class EnvironmentValidator:
    ...
    def validate_all(self) -> Tuple[bool, Dict[str, str]]:
        ...
        if os.getenv("REDIS_URL") or os.getenv("CELERY_BROKER_URL"):
            self.info.append("ℹ️  Celery/Redis configured - transcription features enabled")
        else:
            self.warnings.append("⚠️  No Redis/Celery configured - transcription features disabled")
    ...
    def _validate_var_content(self, var_name: str, value: str) -> Tuple[bool, str]:
        ...
        if var_name == "DATABASE_URL":
            if not value.startswith(("postgresql://", "postgres://")):
                return False, "Invalid database URL format (should start with postgresql://)"
            return True, ""
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        return False, "Invalid credentials format (not valid JSON or base64)"
                ...
                if missing_fields:
                    return False, f"Missing fields in service account JSON: {', '.join(missing_fields)}"
                ...
                if "your-project" in creds.get("project_id", ""):
                    return False, "Using placeholder project ID - please use real credentials"
            ...
            if not value[0].isalnum() or not value[-1].isalnum():
                return False, "Bucket name must start and end with alphanumeric character"
            return True, ""
            ...
            if value == "dev-secret-key":
                if self.environment == "production":
                    ...
                    self.warnings.append(f"⚠️  Using default SECRET_KEY - only for development!")
            return True, ""
