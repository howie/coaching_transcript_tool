...
async def update_user_preferences(
    ...
    return {"message": "Preferences updated successfully", "preferences": current_user.get_preferences()}
...
async def change_password(
    ...
    if current_user.auth_provider != 'email':
        raise HTTPException(status_code=400, detail="Password change not available for SSO users")
    
    if not current_user.hashed_password:
        raise HTTPException(status_code=400, detail="No password set. Use set-password endpoint.")
...
async def get_user_usage(current_user: User = Depends(get_current_user_dependency)):
    ...
    return {
        "current_month": {
            "processed_files": 0,
            "total_size_mb": 0.0,
            "remaining_quota": 60 - current_user.usage_minutes if current_user.plan == UserPlan.FREE else 600 - current_user.usage_minutes
        },
        "total": {
            "processed_files": 0,
            "total_size_mb": 0.0,
