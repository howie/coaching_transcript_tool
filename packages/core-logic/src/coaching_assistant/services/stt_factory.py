...
class STTProviderFactory:
    ...
    def create(provider_type: Optional[Literal["google", "google_stt_v2", "assemblyai", "whisper"]] = None) -> STTProvider:
        ...
        try:
            ...
            raise STTProviderError(f"Failed to create STT provider '{provider_type}': {e}")
    ...
    def create_with_fallback(
        ...
        try:
            logger.info(f"Attempting to create primary STT provider: {primary_provider}")
            return STTProviderFactory.create(primary_provider)
        except Exception as e:
            logger.warning(f"Primary provider '{primary_provider}' failed: {e}. Falling back to '{fallback_provider}'")
