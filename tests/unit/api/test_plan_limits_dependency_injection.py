...
class TestDependencyInjectionCompatibility:
    ...
    def test_user_response_would_cause_attribute_error(self):
        ...
        with pytest.raises(
            AttributeError,
            match="'UserResponse' object has no attribute 'session_count'",
        ):
            _ = user_response.session_count

        with pytest.raises(
            AttributeError,
            match="'UserResponse' object has no attribute 'transcription_count'",
        ):
            _ = user_response.transcription_count

        with pytest.raises(
            AttributeError,
            match="'UserResponse' object has no attribute 'usage_minutes'",
        ):
