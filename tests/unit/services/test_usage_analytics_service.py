...
from coaching_assistant.services.usage_analytics_service import UsageAnalyticsService
...
class TestUsageAnalyticsService:
    ...
    def test_record_usage_snapshot_creates_new(self, analytics_service, test_user, db_session):
        ...
        db_snapshot = db_session.query(UsageHistory).filter(
    ...
    def test_record_usage_snapshot_updates_existing(self, analytics_service, test_user, db_session):
        ...
        updated_snapshot = analytics_service.record_usage_snapshot(test_user.id, "daily")
        ...
        count = db_session.query(UsageHistory).filter(
            UsageHistory.user_id == test_user.id,
            UsageHistory.period_type == "daily"
        ).count()
        assert count == 1
    
    def test_get_usage_trends_with_history_data(self, analytics_service, test_user, db_session):
    ...
    def test_get_usage_trends_fallback_to_logs(self, analytics_service, test_user, db_session):
    ...
    def test_predict_usage_with_sufficient_data(self, analytics_service, test_user, db_session):
        ...
        assert predictions["confidence"] > 0  # Should have some confidence with 30 days of data
    
    def test_predict_usage_insufficient_data(self, analytics_service, test_user, db_session):
    ...
    def test_generate_insights_low_utilization(self, analytics_service, test_user, db_session):
        ...
        for i in range(10):
            history = UsageHistory(
                ...
                plan_name="FREE",
                plan_limits={"minutes": 60, "sessions": 10}  # FREE plan limits
            )
            db_session.add(history)
    ...
    def test_generate_insights_high_utilization(self, analytics_service, test_user, db_session):
    ...
    def test_generate_insights_rapid_growth(self, analytics_service, test_user, db_session):
    ...
    def test_calculate_period_boundaries(self, analytics_service):
        ...
        start, end = analytics_service._calculate_period_boundaries(test_time, "monthly")
        ...
        start, end = analytics_service._calculate_period_boundaries(test_time_dec, "monthly")
    ...
    def test_aggregate_usage_data(self, analytics_service, test_user, db_session):
        ...
        aggregated = analytics_service._aggregate_usage_data(test_user.id, start_date, end_date)
    ...
    def test_generate_plan_recommendation(self, analytics_service):
        ...
        assert "appropriate" in recommendation.lower() or "suited" in recommendation.lower()
