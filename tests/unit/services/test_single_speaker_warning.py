...
class TestSingleSpeakerWarning:
    ...
    def test_single_speaker_detection_warning(self, caplog):
        ...
        with patch(
            ...
            assert (
                len(warning_messages) == 3
            )  # Should have 3 warning messages (including debug message)
            ...
            role_warning = next(
                (
                    msg
                    for msg in warning_messages
                    if "Cannot assign coach/client roles" in msg
                ),
                None,
            )
    ...
    def test_more_speakers_than_expected_info(self, caplog):
        ...
        with patch(
            ...
            more_speakers_info = next(
                (
                    msg
                    for msg in info_messages
                    if "detected more speakers than expected" in msg
                ),
                None,
            )
            ...
            assert metadata["speaker_diarization_mismatch"] is True
            assert (
                metadata["automatic_role_assignment"] is False
