...
class TestCurrentUserEndpoint(TestConfig):
    ...
    def test_get_current_user_success(self, test_client, auth_headers, sample_user_data):
...
class TestGoogleOAuthEndpoints(TestConfig):
    ...
    @patch('httpx.AsyncClient')
    def test_google_callback_success_existing_user(self, mock_client, test_client, created_user):
    ...
    def test_google_callback_network_error(self, mock_client, test_client):
        ...
        mock_client_instance = Mock()
        mock_client_instance.post = AsyncMock(side_effect=httpx.ConnectTimeout("Connection timeout"))
        mock_client.return_value.__aenter__.return_value = mock_client_instance
