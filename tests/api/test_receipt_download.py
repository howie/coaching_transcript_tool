...
class TestReceiptDownloadAPI:
    ...
    def test_generate_receipt_success(
        self, test_client: TestClient, test_user: User, test_db: Session
    ):
        ...
        assert receipt["subscription"]["period_start"] == date.today().strftime(
            "%Y-%m-%d"
        )
        assert receipt["subscription"]["period_end"] == date.today().strftime(
            "%Y-%m-%d"
        )
    ...
    def test_unauthorized_access(self, test_client: TestClient, test_db: Session):
        ...
        test_user = User(id=uuid4(), email="test@example.com", name="Test User")
        ...
        assert (
            ...
            == "Access denied. Payment does not belong to current user."
        )

    def test_failed_payment_receipt(
        self, test_client: TestClient, test_user: User, test_db: Session
    ):
        ...
        assert (
            response.json()["detail"]
            == "Receipt can only be generated for successful payments."
        )
    ...
    def test_receipt_id_format(
        ...
        # Verify receipt ID format: RCP-XXXXXXXX (RCP- + first 8 chars of
        # payment ID)
    ...
    def test_receipt_amount_conversion(
        self, test_client: TestClient, test_user: User, test_db: Session
    ):
