...
class ManualLeMURTester:
    ...
    def test_text_cleanup_functions(self):
        ...
        for i, (input_text, expected) in enumerate(test_cases):
            ...
            print(
                f"  {i + 1:2d}. {status} '{input_text}' → '{result}' (expected: '{expected}')"
            )
        ...
        for i, (input_text, expected_contains) in enumerate(test_cases):
            try:
                result = self.smoother._apply_mandatory_cleanup(
                    input_text, context_language="zh"
                )
                ...
                has_traditional = any(
                    char in result for char in ["個", "關", "沒", "問", "題", "謝"]
                )
    ...
    def test_segment_merging(self):
        ...
        for i, seg in enumerate(test_segments):
            gap = (
                test_segments[i + 1]["start"] - seg["end"]
                if i < len(test_segments) - 1
                else 0
            )
            print(
                f"  {i + 1}. Speaker {seg['speaker']}: '{seg['text']}' (gap to next: {gap}ms)"
            )

        merged = self.smoother._merge_close_segments(
            test_segments, max_gap_ms=5000
        )  # Generous gap for this test
    ...
    def test_response_parsing(self):
        ...
        for test_case in test_responses:
            ...
            try:
                ...
                print(
                    f"  ✅ Parsed {len(segments)} segments (expected ~{test_case['expected_segments']})"
                )
    ...
    async def test_full_pipeline_mock(self):
        ...
        print(
            "   Issues: Spaces between Chinese, simplified Chinese, fragmented content"
        )
...
async def main():
    ...
    try:
        ...
        print(
            "📋 Review the results above to verify improvements are working correctly."
        )
