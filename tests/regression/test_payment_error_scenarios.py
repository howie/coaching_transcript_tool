...
try:
    from coaching_assistant.models import (
        ...
        User, SubscriptionStatus, PaymentStatus, ECPayAuthStatus
    )
except ImportError:
...
try:
    from coaching_assistant.exceptions import (
        ECPayError, CheckMacValueError, PaymentProcessingError
    )
except ImportError:
...
class TestECPayCheckMacValueRegression:
    ...
    def test_checkmacvalue_calculation_with_special_characters(self):
        ...
        try:
            from coaching_assistant.utils.ecpay_utils import generate_check_mac_value
        except ImportError:
        ...
        test_params = {
            ...
            "ReturnURL": "https://example.com/callback?param=value&test=123"  # Contains URL params
        }
        
        # Should generate valid CheckMacValue without throwing exceptions
        mac_value = generate_check_mac_value(test_params, "test_hash_key", "test_hash_iv")
        ...
        mac_value_2 = generate_check_mac_value(test_params, "test_hash_key", "test_hash_iv")
    ...
    def test_checkmacvalue_step7_dotnet_encoding_regression(self):
        ...
        try:
            from coaching_assistant.utils.ecpay_utils import _apply_dotnet_encoding
        except ImportError:
            # Mock implementation for testing
        ...
        for encoded, expected in test_cases:
            ...
            assert result == expected, f"Failed to decode {encoded} to {expected}, got {result}"

    def test_merchant_trade_no_length_regression(self):
        """Regression: Ensure MerchantTradeNo doesn't exceed 20 characters."""
        
        try:
            from coaching_assistant.utils.ecpay_utils import generate_merchant_trade_no
        except ImportError:
            # Mock implementation for testing
            import random
            import string
        ...
        for i in range(100):
            ...
            assert len(trade_no) <= 20, f"MerchantTradeNo too long: {len(trade_no)} chars - {trade_no}"
            assert trade_no.isalnum(), f"MerchantTradeNo contains invalid characters: {trade_no}"

    def test_merchant_member_id_encoding_regression(self):
        """Regression: Ensure MerchantMemberID handles user IDs correctly."""
        
        try:
            from coaching_assistant.utils.ecpay_utils import generate_merchant_member_id
        except ImportError:
            # Mock implementation for testing
            import re
        ...
        for user_id in test_user_ids:
            ...
            assert len(member_id) <= 30, f"MerchantMemberID too long: {len(member_id)} chars"
...
class TestWebhookProcessingRegression:
    ...
    def test_concurrent_webhook_race_condition_regression(self, mock_db_session):
        ...
        # Second call returns existing subscription (created by concurrent request)
    ...
    def test_malformed_webhook_data_regression(self, mock_db_session):
        ...
        # Test various malformed webhook scenarios that previously caused crashes
        malformed_scenarios = [
            {},  # Empty data
        ...
        for malformed_data in malformed_scenarios:
            ...
            except (ValueError, KeyError, AttributeError) as expected_error:
                # These are acceptable errors for malformed data
                pass
            except Exception as unexpected_error:
                pytest.fail(f"Unexpected error for {malformed_data}: {unexpected_error}")
    ...
    def test_payment_retry_infinite_loop_regression(self, mock_db_session):
        ...
        # Create payment with high retry count (potential infinite loop scenario)
...
class TestSubscriptionStatusRegression:
    ...
    def test_grace_period_calculation_regression(self, mock_db_session):
        ...
        for retry_count, expected_days in test_cases:
            ...
            assert time_diff < 3600, f"Grace period miscalculated for retry {retry_count}"
    ...
    def test_subscription_status_transition_regression(self, mock_db_session):
        ...
        valid_transitions = [
            (SubscriptionStatus.ACTIVE, SubscriptionStatus.PAST_DUE),     # Payment failed
            (SubscriptionStatus.PAST_DUE, SubscriptionStatus.ACTIVE),     # Payment recovered
            (SubscriptionStatus.PAST_DUE, SubscriptionStatus.CANCELLED),  # Max failures
            (SubscriptionStatus.ACTIVE, SubscriptionStatus.CANCELLED),    # User cancellation
        ]
        ...
        for from_status, to_status in valid_transitions:
            ...
            assert result is True, f"Should allow transition from {from_status} to {to_status}"

        # Test invalid transitions that should be prevented
        invalid_transitions = [
            (SubscriptionStatus.CANCELLED, SubscriptionStatus.ACTIVE),    # Can't reactivate cancelled
            (SubscriptionStatus.CANCELLED, SubscriptionStatus.PAST_DUE),  # Can't set cancelled to past due
        ]
        ...
        for from_status, to_status in invalid_transitions:
            ...
            result = service._validate_status_transition(mock_subscription, to_status)
            assert result is False, f"Should prevent transition from {from_status} to {to_status}"
...
class TestPaymentAmountRegression:
    ...
    def test_currency_conversion_precision_regression(self):
        ...
        try:
            from coaching_assistant.utils.currency_utils import twd_to_cents, cents_to_twd
        except ImportError:
            # Mock implementation for testing
        ...
        for twd, expected_cents in test_amounts:
            ...
            assert cents == expected_cents, f"TWD {twd} should convert to {expected_cents} cents, got {cents}"
            
            # Reverse conversion
            converted_back = cents_to_twd(cents)
            assert abs(converted_back - twd) < 0.01, f"Precision loss in round-trip conversion: {twd} → {cents} → {converted_back}"
    ...
    def test_proration_calculation_regression(self, mock_db_session):
        ...
        try:
            from coaching_assistant.services.billing_service import BillingService
        except ImportError:
            # Mock implementation for testing
    ...
    def test_annual_discount_calculation_regression(self):
        ...
        try:
            from coaching_assistant.utils.pricing_utils import calculate_annual_discount
        except ImportError:
            # Mock implementation for testing
...
class TestSecurityRegression:
    ...
    def test_sql_injection_prevention_regression(self, mock_db_session):
        ...
        for payload in sql_injection_payloads:
            ...
            # Should not cause SQL injection - either processes safely or fails gracefully
            try:
                result = service._process_billing_webhook(webhook_data)
                # If it processes, should not return unexpected data
                assert isinstance(result, (bool, type(None)))
            except (ValueError, SecurityError) as expected:
    ...
    def test_xss_prevention_regression(self):
        ...
        try:
            from coaching_assistant.utils.sanitization import sanitize_user_input
        except ImportError:
            # Mock implementation for testing
            import re
    ...
    def test_admin_token_validation_regression(self, mock_db_session):
        ...
        try:
            from coaching_assistant.api.webhooks.ecpay import validate_admin_token
        except ImportError:
            # Mock implementation for testing
