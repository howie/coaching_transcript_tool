...
class TestTranscriptionPolling:
    ...
    def test_poll_error_status(self, mock_get):
        ...
        with patch('coaching_assistant.services.assemblyai_stt.settings') as mock_settings:
            ...
            with pytest.raises(STTProviderError, match="Transcription failed: Audio too short"):
    ...
    def test_parse_chinese_result_with_conversion(self):
        ...
        with patch('coaching_assistant.services.assemblyai_stt.settings') as mock_settings:
            ...
            with patch.object(provider, '_process_chinese_text') as mock_process:
                ...
                assert parsed.provider_metadata["post_processing_applied"] == ["remove_spaces", "convert_to_traditional"]
        ...
        with patch('coaching_assistant.services.assemblyai_stt.settings') as mock_settings:
            ...
            result = {
                ...
                "words": [
                    ...
                    {"text": "transcript.", "start": 1500, "end": 2500, "confidence": 0.88},
...
class TestFullTranscriptionWorkflow:
    ...
    def test_complete_transcription_workflow(self, mock_sleep, mock_post, mock_get, mock_assign):
        ...
        with patch('coaching_assistant.services.assemblyai_stt.settings') as mock_settings:
            ...
            assert result.segments[1].content == "I'm feeling quite overwhelmed with work lately."
...
class TestGCSIntegration:
    ...
    def test_gcs_uri_to_signed_url(self, mock_gcs_uploader_class):
        ...
        with patch('coaching_assistant.services.assemblyai_stt.settings') as mock_settings:
            ...
            mock_uploader = Mock()
            mock_uploader.generate_signed_read_url.return_value = "https://storage.googleapis.com/bucket/path?signed"
    ...
    def test_gcs_uri_uploader_error(self, mock_gcs_uploader_class):
        ...
        with patch('coaching_assistant.services.assemblyai_stt.settings') as mock_settings:
            ...
            with pytest.raises(STTProviderError, match="Failed to convert GCS URI to signed URL"):
