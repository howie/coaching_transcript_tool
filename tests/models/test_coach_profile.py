...
from coaching_assistant.models import (
    CoachProfile, CoachingPlan, User, UserPlan,
    CoachingLanguage, CommunicationTool, CoachExperience, CoachingPlanType
)
...
class TestCoachingPlan:
    ...
    def test_coaching_plan_relationship_with_profile(self, db_session: Session):
        ...
        profile = CoachProfile(
            user_id=user.id,
            display_name="Relationship Plan Coach"
        )
        db_session.add(profile)
        db_session.commit()
