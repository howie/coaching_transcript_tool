...
try:
    from coaching_assistant.services.monitoring_service import PaymentMonitoringService
except ImportError:
...
try:
    from coaching_assistant.models import (
        SubscriptionPayment, SaasSubscription, WebhookLog,
...
class TestPaymentSuccessRateMonitoring:
    ...
    def test_payment_success_rate_calculation(self, monitoring_service, mock_db_session):
        """Test accurate payment success rate calculation."""
        
        # Mock payment data for last 24 hours
        mock_payments = [
            Mock(status=PaymentStatus.SUCCESS.value, created_at=datetime.now() - timedelta(hours=1)),
            Mock(status=PaymentStatus.SUCCESS.value, created_at=datetime.now() - timedelta(hours=2)),
            Mock(status=PaymentStatus.SUCCESS.value, created_at=datetime.now() - timedelta(hours=3)),
            Mock(status=PaymentStatus.FAILED.value, created_at=datetime.now() - timedelta(hours=4)),
            Mock(status=PaymentStatus.SUCCESS.value, created_at=datetime.now() - timedelta(hours=5)),
        ]
        
        mock_db_session.query.return_value.filter.return_value.all.return_value = mock_payments
        
        # Calculate success rate
        success_rate = monitoring_service.calculate_payment_success_rate(period_hours=24)
    ...
    def test_payment_success_rate_over_time(self, monitoring_service, mock_db_session):
        ...
        last_hour = [
            Mock(status=PaymentStatus.SUCCESS.value, created_at=now - timedelta(minutes=30)),
            Mock(status=PaymentStatus.SUCCESS.value, created_at=now - timedelta(minutes=45)),
        ]
        
        # Last 24 hours: 8 success, 2 failures = 80%
        last_24_hours = last_hour + [
            Mock(status=PaymentStatus.SUCCESS.value, created_at=now - timedelta(hours=2)),
            Mock(status=PaymentStatus.SUCCESS.value, created_at=now - timedelta(hours=4)),
            Mock(status=PaymentStatus.SUCCESS.value, created_at=now - timedelta(hours=6)),
            Mock(status=PaymentStatus.SUCCESS.value, created_at=now - timedelta(hours=8)),
            Mock(status=PaymentStatus.SUCCESS.value, created_at=now - timedelta(hours=12)),
            Mock(status=PaymentStatus.SUCCESS.value, created_at=now - timedelta(hours=18)),
            Mock(status=PaymentStatus.FAILED.value, created_at=now - timedelta(hours=10)),
            Mock(status=PaymentStatus.FAILED.value, created_at=now - timedelta(hours=20)),
        ]
    ...
    def test_success_rate_threshold_alerting(self, monitoring_service, mock_db_session):
        ...
        mock_payments = [
            Mock(status=PaymentStatus.SUCCESS.value) for _ in range(6)
        ] + [
    ...
    def test_webhook_processing_time_monitoring(self, monitoring_service, mock_db_session):
        """Test monitoring of webhook processing times."""
        
        # Mock webhook logs with processing times
        mock_webhook_logs = [
            Mock(processing_time_ms=150, status='success', created_at=datetime.now() - timedelta(minutes=5)),
            Mock(processing_time_ms=230, status='success', created_at=datetime.now() - timedelta(minutes=10)),
            Mock(processing_time_ms=89, status='success', created_at=datetime.now() - timedelta(minutes=15)),
            Mock(processing_time_ms=1200, status='success', created_at=datetime.now() - timedelta(minutes=20)),  # Slow
            Mock(processing_time_ms=340, status='success', created_at=datetime.now() - timedelta(minutes=25)),
        ]
        
        mock_db_session.query.return_value.filter.return_value.all.return_value = mock_webhook_logs
        
        # Calculate average processing time
        avg_processing_time = monitoring_service.calculate_average_webhook_processing_time(period_hours=1)
        
        # Expected: (150 + 230 + 89 + 1200 + 340) / 5 = 401.8ms
        expected_avg = (150 + 230 + 89 + 1200 + 340) / 5
        assert abs(avg_processing_time - expected_avg) < 0.1, f"Expected ~{expected_avg}ms, got {avg_processing_time}ms"
    ...
    def test_webhook_failure_rate_monitoring(self, monitoring_service, mock_db_session):
        ...
        mock_db_session.query.return_value.filter.return_value.all.return_value = mock_webhooks
        
        # Calculate failure rate
        failure_rate = monitoring_service.calculate_webhook_failure_rate(period_hours=24)
        
        # Expected: 2 failures out of 6 total = 33.33%
        expected_rate = (2 / 6) * 100
        assert abs(failure_rate - expected_rate) < 0.1, f"Expected ~{expected_rate}%, got {failure_rate}%"

    def test_subscription_churn_rate_monitoring(self, monitoring_service, mock_db_session):
    ...
    def test_revenue_metrics_monitoring(self, monitoring_service, mock_db_session):
        ...
        mock_subscriptions = [
            Mock(plan_id='PRO', amount_twd=89900, status=SubscriptionStatus.ACTIVE.value),      # $899 TWD
            Mock(plan_id='PRO', amount_twd=89900, status=SubscriptionStatus.ACTIVE.value),
            Mock(plan_id='ENTERPRISE', amount_twd=299900, status=SubscriptionStatus.ACTIVE.value),  # $2999 TWD
            Mock(plan_id='PRO', amount_twd=89900, status=SubscriptionStatus.ACTIVE.value),
            Mock(plan_id='ENTERPRISE', amount_twd=299900, status=SubscriptionStatus.ACTIVE.value),
        ]
        
        mock_db_session.query.return_value.filter.return_value.all.return_value = mock_subscriptions
    ...
    def test_conversion_rate_monitoring(self, monitoring_service, mock_db_session):
        ...
        mock_conversions = [
            Mock(converted_at=datetime.now() - timedelta(days=5), from_plan='FREE', to_plan='PRO'),
            Mock(converted_at=datetime.now() - timedelta(days=10), from_plan='FREE', to_plan='ENTERPRISE'),
        ]
        
        mock_new_users = 10  # Total new users in period
        
        # Mock queries
        mock_db_session.query.return_value.filter.return_value.count.side_effect = [2, 10]  # conversions, new users
        
        conversion_rate = monitoring_service.calculate_conversion_rate(period_days=30)
        
        # Expected: 2 conversions out of 10 new users = 20%
        assert conversion_rate == 20.0, f"Expected 20% conversion rate, got {conversion_rate}%"
...
class TestRealTimeMonitoring:
    ...
    def test_real_time_payment_status_tracking(self, monitoring_service, mock_db_session):
    ...
    def test_webhook_response_time_tracking(self, monitoring_service):
        ...
        with patch.object(monitoring_service, 'record_webhook_processing_time') as mock_record:
            monitoring_service.track_webhook_processing("billing_callback", processing_time, "success")
            
            mock_record.assert_called_once_with("billing_callback", processing_time, "success")
        ...
        with patch.object(monitoring_service, 'send_alert') as mock_alert:
            for scenario in alert_scenarios:
                ...
                if scenario["should_alert"]:
                    ...
                    assert mock_alert.called, f"Should alert for {scenario['metric']} = {scenario['value']}"
...
class TestMonitoringDashboard:
    ...
    def test_dashboard_metrics_aggregation(self, monitoring_service, mock_db_session):
        ...
        with patch.object(monitoring_service, 'calculate_payment_success_rate', return_value=97.5):
            with patch.object(monitoring_service, 'calculate_average_webhook_processing_time', return_value=245.0):
                with patch.object(monitoring_service, 'calculate_monthly_churn_rate', return_value=4.2):
                    with patch.object(monitoring_service, 'calculate_monthly_recurring_revenue', return_value=125000):
                        with patch.object(monitoring_service, 'calculate_conversion_rate', return_value=18.5):
                            ...
                            assert dashboard_data['avg_webhook_processing_time'] == 245.0
                            ...
                            assert dashboard_data['overall_health'] in ['healthy', 'warning', 'critical']
    ...
    def test_historical_metrics_trending(self, monitoring_service, mock_db_session):
        ...
        historical_success_rates = [98.5, 97.8, 99.1, 96.5, 97.2, 98.9, 97.5]  # Last 7 days
        
        with patch.object(monitoring_service, 'get_historical_success_rates', return_value=historical_success_rates):
    ...
    def test_alert_summary_generation(self, monitoring_service):
        ...
        with patch.object(monitoring_service, 'get_recent_alerts', return_value=mock_alerts):
