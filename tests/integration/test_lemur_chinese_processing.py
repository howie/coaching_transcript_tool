...
class TestLeMURChineseProcessingIntegration:
    ...
    async def test_combined_processing_with_pure_text_response(self, mock_lemur_task):
        ...
        # Mock LeMUR response - pure text format (what often happens in practice)
        mock_response = Mock()
    ...
    async def test_emergency_fallback_when_lemur_fails(self, mock_lemur_task):
        ...
        for segment in result.segments:
            ...
            if original_had_spaces:
                ...
                assert segment.text != self.test_segments[result.segments.index(segment)]['text']
    
    @pytest.mark.asyncio
    @patch('coaching_assistant.utils.chinese_converter.convert_to_traditional')
    @patch('coaching_assistant.services.lemur_transcript_smoother.aai.Lemur.task')
    async def test_mandatory_traditional_chinese_conversion(self, mock_lemur_task, mock_convert):
        """Test that Traditional Chinese conversion is always applied."""
        
        # Mock conversion
        mock_convert.side_effect = lambda x: x.replace('关于', '關於').replace('问题', '問題')
    ...
    def test_segment_merging_before_lemur(self):
        ...
        fragmented_segments = [
            {'speaker': 'A', 'text': '我们今天就先', 'start': 1000, 'end': 2000},
            {'speaker': 'A', 'text': '暂时结束在这边', 'start': 2100, 'end': 2500},  # 100ms gap
            {'speaker': 'B', 'text': 'OK我没有问题', 'start': 2600, 'end': 3000}
        ]
...
class TestConvenienceFunctionIntegration:
    ...
    async def test_convenience_function_with_combined_mode(self, mock_lemur_task):
        ...
        segments = [
            {'speaker': 'A', 'text': '测试 内容 一', 'start': 1000, 'end': 2000},
            {'speaker': 'B', 'text': '测试 内容 二', 'start': 2000, 'end': 3000}
        ]
