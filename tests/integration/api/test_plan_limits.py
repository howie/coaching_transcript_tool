...
class TestPlanValidationAPI:
    ...
    @pytest.mark.parametrize("action,params,expected_type", [
    ...
    def test_validate_multiple_action_types(
        self, client: TestClient, free_user: User, auth_headers: dict,
        action: str, params: dict, expected_type: str
    ):
...
class TestUsageTrackingIntegration:
    ...
    def test_usage_increments_after_action(
        self, client: TestClient, free_user: User, auth_headers: dict, db_session: Session
    ):
    ...
    def test_monthly_reset(
        self, client: TestClient, maxed_free_user: User, auth_headers: dict, db_session: Session
    ):
        ...
        with patch('coaching_assistant.services.usage_tracker.datetime') as mock_datetime:
            ...
            from coaching_assistant.tasks.usage_tasks import reset_monthly_usage
            reset_monthly_usage()
...
class TestValidationPerformance:
    ...
    def test_validation_response_time(
        self, client: TestClient, free_user: User, auth_headers: dict, benchmark
    ):
...
class TestValidationErrorHandling:
    ...
    def test_validation_database_error(
        ...
        with patch('coaching_assistant.services.usage_tracker.UsageTracker.get_current_usage') as mock_get:
