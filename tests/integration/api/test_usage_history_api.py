...
class TestUsageHistoryAPI:
    ...
    def test_get_usage_history_success(self, client: TestClient, test_user, sample_usage_history, auth_headers, mock_auth):
    ...
    def test_get_usage_history_custom_period(self, client: TestClient, test_user, sample_usage_history, auth_headers, mock_auth):
    ...
    def test_get_usage_trends_success(self, client: TestClient, test_user, sample_usage_history, auth_headers, mock_auth):
        """Test getting usage trends successfully."""
        mock_auth.return_value = test_user
        
        response = client.get(
            "/api/v1/usage/trends?period=7d&group_by=day",
            headers=auth_headers
        )
    ...
    def test_get_usage_predictions_success(self, client: TestClient, test_user, sample_usage_history, auth_headers, mock_auth):
    ...
    def test_get_usage_insights_success(self, client: TestClient, test_user, sample_usage_history, auth_headers, mock_auth):
        ...
        if data:  # If we have insights
            ...
            assert first_insight["type"] in ["pattern", "optimization", "warning", "trend", "cost"]
            assert first_insight["priority"] in ["low", "medium", "high"]
            if first_insight["action"]:
                assert first_insight["action"] in ["upgrade", "downgrade", "review"]
    
    def test_get_comprehensive_analytics_success(self, client: TestClient, test_user, sample_usage_history, auth_headers, mock_auth):
    ...
    def test_create_usage_snapshot_success(self, client: TestClient, test_user, auth_headers, mock_auth, db_session):
        ...
        response = client.post(
            "/api/v1/usage/snapshot?period_type=daily",
            headers=auth_headers
        )
    ...
    def test_export_usage_data_json(self, client: TestClient, test_user, sample_usage_history, auth_headers, mock_auth):
        """Test exporting usage data in JSON format."""
        mock_auth.return_value = test_user
        
        response = client.get(
            "/api/v1/usage/export?format=json&period=7d",
            headers=auth_headers
        )
    ...
    def test_export_usage_data_csv_not_implemented(self, client: TestClient, test_user, auth_headers, mock_auth):
        """Test that CSV export returns not implemented message."""
        mock_auth.return_value = test_user
        
        response = client.get(
            "/api/v1/usage/export?format=csv&period=7d",
            headers=auth_headers
        )
    ...
    def test_export_usage_data_invalid_format(self, client: TestClient, test_user, auth_headers, mock_auth):
    ...
    def test_unauthorized_access(self, client: TestClient):
        ...
        for endpoint in endpoints:
            ...
            assert response.status_code in [401, 403, 422]  # 422 for missing dependency
    
    def test_usage_history_with_no_data(self, client: TestClient, test_user, auth_headers, mock_auth):
    ...
    def test_trends_with_different_group_by(self, client: TestClient, test_user, sample_usage_history, auth_headers, mock_auth):
        ...
        response = client.get("/api/v1/usage/trends?group_by=week", headers=auth_headers)
        assert response.status_code == 200
        
        # Test monthly grouping
        response = client.get("/api/v1/usage/trends?group_by=month", headers=auth_headers)
        assert response.status_code == 200
    
    def test_analytics_different_periods(self, client: TestClient, test_user, sample_usage_history, auth_headers, mock_auth):
        ...
        for period in periods:
            response = client.get(f"/api/v1/usage/analytics?period={period}", headers=auth_headers)
            assert response.status_code == 200
            data = response.json()
    ...
    def test_error_handling(self, client: TestClient, test_user, auth_headers, mock_auth, db_session):
    ...
    def test_snapshot_different_period_types(self, client: TestClient, test_user, auth_headers, mock_auth):
