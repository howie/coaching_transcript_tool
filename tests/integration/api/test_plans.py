...
class TestPlansAPI:
    ...
    def test_get_current_plan_status_enterprise_user(
        ...
        assert (
            data["usageStatus"]["usagePercentages"]["sessions"] is None
        )  # No percentage for unlimited
        assert (
            "upgradeSuggestion" not in data["usageStatus"]
        )  # No upgrade from enterprise
    ...
    def test_validate_action_transcribe_limit(
        ...
        response = authenticated_client.post(
            "/api/v1/plans/validate", json={"action": "transcribe"}
        )
        assert response.status_code == 200
    ...
    def test_validate_action_enterprise_unlimited(
        ...
        for action_data in actions:
            response = authenticated_client.post(
                "/api/v1/plans/validate", json=action_data
            )
            assert response.status_code == 200
            data = response.json()
            assert data["allowed"] is True, (
                f"Action {action_data} should be allowed for enterprise"
            )
    ...
    def test_unauthorized_access(self, client: TestClient):
        ...
        endpoints = ["/api/v1/plans/current", "/api/v1/plans/compare"]
