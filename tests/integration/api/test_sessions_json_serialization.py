...
def test_session_limit_exceeded_json_serialization(
...
def test_file_size_exceeded_json_serialization(
    authenticated_client, db_session, test_user
):
...
def test_transcription_limit_exceeded_json_serialization(
    authenticated_client, db_session, test_user
):
    ...
    response = authenticated_client.post(
        f"/api/v1/sessions/{session_id}/start-transcription"
    )
...
def test_upload_url_requires_file_size_mb_parameter(
    authenticated_client, db_session, test_user
):
    ...
    missing_fields = [
        error["loc"][-1] for error in validation_errors if error["type"] == "missing"
    ]
    assert "file_size_mb" in missing_fields


def test_upload_url_success_with_file_size_mb(
    authenticated_client, db_session, test_user
):
...
def test_upload_url_filename_validation(authenticated_client, db_session, test_user):
    ...
    for ext in valid_extensions:
        response = authenticated_client.post(
            f"/api/v1/sessions/{session_id}/upload-url",
            params={"filename": f"test.{ext}", "file_size_mb": 10},
        )
