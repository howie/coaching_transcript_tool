...
class TestUsageAPI:
    ...
    @patch(
        "coaching_assistant.services.usage_tracking.UsageTrackingService.get_user_usage_summary"
    )
    ...
    @patch(
        "coaching_assistant.services.usage_tracking.UsageTrackingService.get_user_usage_history"
    )
    ...
    def test_get_usage_history_validation(self, client: TestClient, auth_headers: dict):
    ...
    @patch(
        "coaching_assistant.services.usage_tracking.UsageTrackingService.get_user_analytics"
    )
    ...
    @patch(
        "coaching_assistant.services.usage_tracking.UsageTrackingService.get_admin_analytics"
    )
    def test_admin_analytics_success(
        ...
        mock_analytics = {
            ...
            "total_minutes": 60000,
            "plan_distribution": {"free": 70, "pro": 25, "enterprise": 5},
        }
        mock_get_analytics.return_value = mock_analytics
    ...
    def test_admin_get_specific_user_usage(
        ...
        with patch(
            ...
            response = client.get(
                f"/api/usage/admin/user/{test_user.id}", headers=admin_headers
            )
            assert response.status_code == 200
    ...
    def test_admin_get_nonexistent_user_usage(
        ...
        response = client.get(
            f"/api/usage/admin/user/{fake_user_id}", headers=admin_headers
        )
