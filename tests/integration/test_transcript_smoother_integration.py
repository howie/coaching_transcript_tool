...
class TestRealWorldIntegration:
    ...
    def test_chinese_coaching_session_transcript(self):
        ...
        # Should have processed the transcript (may merge into fewer segments
        # due to smoothing)
        assert len(segments) >= 1
        assert stats["language_detected"] == "chinese"
        assert stats["processor_used"] == "ChineseProcessor"

        # Check processing occurred (may be short head backfill instead of
        # filler word)
        assert stats["moved_word_count"] >= 0
        assert (
            stats["heuristic_hits"]["short_first_segment"]
            + stats["heuristic_hits"]["filler_words"]
            >= 1
        )
    ...
    def test_complex_punctuation_scenarios(self):
        ...
        segments = result["segments"]
        found_smart_quotes = any(
            '"' in segment["text"] and '"' in segment["text"] for segment in segments
        )
...
class TestConfigurationOptions:
    ...
    def test_threshold_adjustments(self):
        ...
        result_strict = smooth_and_punctuate(
            transcript_json, th_short_head_sec=0.1, th_filler_max_sec=0.1
        )

        # Test with very permissive thresholds (should allow merging)
        result_permissive = smooth_and_punctuate(
            transcript_json, th_short_head_sec=2.0, th_filler_max_sec=1.0
        )

        # Results should be different
        assert (
            result_strict["stats"]["moved_word_count"]
            != result_permissive["stats"]["moved_word_count"]
        )
