...
async def test_lemur_integration():
    ...
    try:
        ...
        if not api_key:
            ...
            logger.info(
                "💡 Set the API key with: export ASSEMBLYAI_API_KEY='your_key_here'"
            )
            return False
        ...
        success_indicators = [
            ...
            any(
                "教練" in segment.speaker or "Coach" in segment.speaker
                for segment in result.segments
            ),
            any(
                "客戶" in segment.speaker or "Client" in segment.speaker
                for segment in result.segments
            ),
        ]
        ...
        if all(success_indicators):
            ...
            logger.warning(
                f"   Speaker mapping: {len(result.speaker_mapping)} (expected > 0)"
            )
            logger.warning(
                f"   Improvements: {len(result.improvements_made)} (expected > 0)"
            )
            return False
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                logger.info(
                    f"ℹ️  API endpoint not available (status: {response.status_code})"
                )
...
async def main():
    ...
    logger.info(
        f"   Core LeMUR functionality: {'✅ PASSED' if test1_passed else '❌ FAILED'}"
    )
