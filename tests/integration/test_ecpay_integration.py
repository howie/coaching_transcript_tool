...
class TestECPayIntegration:
    ...
    def test_merchant_trade_no_length_compliance(self, ecpay_service):
        ...
        for user_id in test_cases:
            ...
            assert merchant_trade_no.startswith("SUB"), (
                "MerchantTradeNo must start with SUB"
            )
            assert len(merchant_trade_no) >= 3, "MerchantTradeNo too short"
            assert merchant_trade_no.replace("SUB", "").replace("-", "").isalnum(), (
                "Invalid characters"
            )
        ...
        with patch.object(ecpay_service, "_get_plan_pricing") as mock_pricing:
            ...
            try:
                ...
                for field in required_fields:
                    ...
                    assert str(auth_data[field]).strip() != "", (
                        f"Field {field} is empty"
                    )

                # Specific value validations
                assert auth_data["MerchantTradeNo"] != "", (
                    "MerchantTradeNo cannot be empty"
                )
                assert len(auth_data["MerchantTradeNo"]) <= 20, (
                    "MerchantTradeNo too long"
                )
        ...
        for billing_cycle, expected_period_type in test_cases:
            with patch.object(ecpay_service, "_get_plan_pricing") as mock_pricing:
                ...
                try:
                    ...
                    pytest.fail(
                        f"Authorization creation failed for {billing_cycle}: {e}"
                    )
    ...
    def test_uniqueness_constraints(self, ecpay_service):
        ...
        with patch.object(ecpay_service, "_get_plan_pricing") as mock_pricing:
            ...
            for i in range(10):
                try:
                    ...
                    # Check uniqueness
                    assert trade_no not in generated_trade_nos, (
                        f"Duplicate MerchantTradeNo: {trade_no}"
                    )
                    assert member_id not in generated_member_ids, (
                        f"Duplicate MerchantMemberID: {member_id}"
                    )
        ...
        assert len(generated_trade_nos) == 10, (
            "Not all MerchantTradeNo values were unique"
        )
        assert len(generated_member_ids) == 10, (
            "Not all MerchantMemberID values were unique"
        )
