...
class LeMURDatabaseTester:
    ...
    async def list_available_sessions(self, limit: int = 20) -> List[Dict]:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            logger.info(
                f"📊 Found {len(sessions)} total sessions, {len(completed_sessions)} completed"
            )
    ...
    async def get_session_details(self, session_id: str) -> Dict:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if response.status_code != 200:
                raise RuntimeError(
                    f"Failed to get session details: {response.status_code} - {response.text}"
                )

            session_data = response.json()
            logger.info(
                f"✅ Session found - Title: {session_data.get('title', 'Untitled')}"
            )
    ...
    async def apply_speaker_identification(
        self, session_id: str, custom_prompt: Optional[str] = None
    ) -> Dict:
        ...
        async with httpx.AsyncClient(timeout=120.0) as client:
            ...
            logger.info(
                f"🎭 Applying LeMUR speaker identification to session: {session_id}"
            )
            ...
            if response.status_code != 200:
                raise RuntimeError(
                    f"Failed to apply speaker identification: {response.status_code} - {response.text}"
                )
    ...
    async def apply_punctuation_optimization(
        self, session_id: str, custom_prompt: Optional[str] = None
    ) -> Dict:
        ...
        async with httpx.AsyncClient(timeout=120.0) as client:
            ...
            logger.info(
                f"🔤 Applying LeMUR punctuation optimization to session: {session_id}"
            )
            response = await client.post(
            ...
            if response.status_code != 200:
                raise RuntimeError(
                    f"Failed to apply punctuation optimization: {response.status_code} - {response.text}"
                )
            ...
            logger.info("✅ Punctuation optimization completed:")
            logger.info(f"   Database updates: {content_updates}")
            logger.info(f"   Improvements: {improvements}")

            return result

    def analyze_segment_changes(
        self, before_segments: List[Dict], after_segments: List[Dict]
    ) -> Dict:
        ...
        if len(before_segments) != len(after_segments):
            logger.warning(
                f"⚠️  Segment count mismatch: {len(before_segments)} → {len(after_segments)}"
            )
        ...
        if speaker_changes:
            ...
            for change in speaker_changes[:3]:
                ...
                logger.info(
                    f"     Speaker: {change['before_speaker']} → {change['after_speaker']}"
                )
        ...
        if content_changes:
            ...
            for change in content_changes[:3]:
                logger.info(
                    f"   Segment {change['segment_index'] + 1} (Speaker {
                        change['speaker']
                    }):"
                )
                logger.info(f"     Before: {change['before_content']}")
                logger.info(f"     After:  {change['after_content']}")
                logger.info("     Change: Punctuation/formatting improved")
    ...
    async def test_database_processing(
        ...
        try:
            ...
            if session_details.get("status") != "completed":
                raise ValueError(
                    f"Session status is '{session_details.get('status')}', expected 'completed'"
                )
            ...
            if not skip_speaker_identification:
                speaker_result = await self.apply_speaker_identification(
                    session_id, custom_speaker_prompt
                )

                # Get segments after speaker identification
                after_speaker_segments = await self.get_transcript_segments(session_id)
                speaker_analysis = self.analyze_segment_changes(
                    initial_segments, after_speaker_segments
                )

                logger.info("\n🎭 SPEAKER IDENTIFICATION ANALYSIS:")
                logger.info(
                    f"   Database updates made: {
                        speaker_result.get('segment_updates', 0)
                    }"
                )
                logger.info(
                    f"   Actual speaker changes detected: {speaker_analysis['speaker_changes']}"
                )

            if not skip_punctuation_optimization:
                # Use segments after speaker identification as baseline for
                # punctuation
                baseline_segments = (
                    after_speaker_segments
                    if not skip_speaker_identification
                    else initial_segments
                )

                punctuation_result = await self.apply_punctuation_optimization(
                    session_id, custom_punctuation_prompt
                )

                # Get final segments after punctuation optimization
                final_segments = await self.get_transcript_segments(session_id)
                punctuation_analysis = self.analyze_segment_changes(
                    baseline_segments, final_segments
                )

                logger.info("\n🔤 PUNCTUATION OPTIMIZATION ANALYSIS:")
                logger.info(
                    f"   Database updates made: {
                        punctuation_result.get('segment_updates', 0)
                    }"
                )
                logger.info(
                    f"   Actual content changes detected: {punctuation_analysis['content_changes']}"
                )

            # Step 4: Final comparison
            final_segments = await self.get_transcript_segments(session_id)
            overall_analysis = self.analyze_segment_changes(
...
async def main():
    parser = argparse.ArgumentParser(
        description="Test LeMUR database processing on existing sessions"
    )
    ...
    parser.add_argument(
        "--list-sessions",
        action="store_true",
        help="List available completed sessions",
    )
    parser.add_argument("--speaker-prompt", help="Custom speaker identification prompt")
    parser.add_argument(
        "--punctuation-prompt", help="Custom punctuation optimization prompt"
    )
    parser.add_argument(
        "--skip-speaker",
        action="store_true",
    ...
    parser.add_argument(
        "--skip-punctuation",
        action="store_true",
        help="Skip punctuation optimization",
    )
    ...
    try:
        ...
        if not args.session_id:
            logger.error(
                "❌ Please provide --session-id or use --list-sessions to see available sessions"
            )
            sys.exit(1)
