...
class TestPlanLimitsE2E:
    ...
    def test_plan_comparison_endpoint(self):
        ...
        user = self.auth.create_test_user(
            self.db, "compare_test@test.com", UserPlan.FREE
        )
    ...
    def test_current_plan_status(self):
        ...
        # Test Pro user
        pro_user = self.auth.create_test_user(
            self.db, "pro_status@test.com", UserPlan.PRO
        )
    ...
    def test_plan_validation_endpoint(self):
        ...
        user = self.auth.create_test_user(
            self.db, "validation_test@test.com", UserPlan.FREE
        )
        self.auth.override_auth_dependency(app, user)

        # Test session creation validation - should be allowed
        response = self.client.post(
            "/api/v1/plans/validate", json={"action": "create_session"}
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_v1_plan_validation_endpoint(self):
        ...
        user = self.auth.create_test_user(
            self.db, "v1_validation@test.com", UserPlan.FREE
        )
        self.auth.override_auth_dependency(app, user)
        ...
        response = self.client.post(
            "/api/v1/plan/validate-action",
            json={"action": "upload_file", "params": {"file_size_mb": 20}},
        )
        assert response.status_code == 200
        data = response.json()
        ...
        response = self.client.post(
            "/api/v1/plan/validate-action",
            json={"action": "upload_file", "params": {"file_size_mb": 30}},
        )
        assert response.status_code == 200
        data = response.json()
        assert data["allowed"] is False
    ...
    def test_export_format_validation(self):
        ...
        # Free user
        free_user = self.auth.create_test_user(
            self.db, "free_export@test.com", UserPlan.FREE
        )
        self.auth.override_auth_dependency(app, free_user)
        ...
        response = self.client.post(
            ...
            json={"action": "export_transcript", "format": "txt"},
        )
        assert response.status_code == 200
        data = response.json()
        assert data["allowed"] is True
        ...
        response = self.client.post(
            "/api/v1/plans/validate",
            json={"action": "export_transcript", "format": "xlsx"},
        )
        assert response.status_code == 200
        data = response.json()
        assert data["allowed"] is False
        ...
        pro_user = self.auth.create_test_user(
            self.db, "pro_export@test.com", UserPlan.PRO
        )
        self.auth.override_auth_dependency(app, pro_user)

        response = self.client.post(
            "/api/v1/plans/validate",
            json={"action": "export_transcript", "format": "vtt"},
        )
        assert response.status_code == 200
        data = response.json()
        assert data["allowed"] is True
    ...
    def test_enterprise_unlimited_features(self):
        ...
        for format in ["txt", "json", "vtt", "srt", "xlsx"]:
            response = self.client.post(
                "/api/v1/plans/validate",
                json={"action": "export_transcript", "format": format},
            )
            assert response.status_code == 200
            assert response.json()["allowed"] is True
    ...
    def test_usage_near_limit_helper(self):
        ...
        user = self.auth.create_test_user(
            self.db, "helper_test@test.com", UserPlan.FREE
        )
