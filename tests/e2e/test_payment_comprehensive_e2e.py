...
class TestPaymentFlowsE2E:
    ...
    def test_complete_subscription_authorization_flow(
        self, test_client, mock_user_token, subscription_test_data
    ):
        ...
        for plan in subscription_test_data["plans"]:
            for cycle in subscription_test_data["cycles"]:
                ...
                auth_request = {"plan_id": plan, "billing_cycle": cycle}

                with patch(
                    "src.coaching_assistant.services.auth_service.verify_jwt_token"
                ) as mock_auth:
                    ...
                    if response.status_code == 200:
                        ...
                        self._validate_ecpay_authorization_response(
                            auth_data, plan, cycle
                        )
    ...
    def _validate_ecpay_authorization_response(
        self, auth_data: Dict[str, Any], plan: str, cycle: str
    ):
    ...
    def _simulate_ecpay_payment_completion(
        self, client: TestClient, member_id: str, trade_no: str
    ):
    ...
    def _create_test_subscription_with_failure(
    ...
    def test_concurrent_webhook_processing(self, test_client):
        ...
        assert len(successful_webhooks) >= 3, (
            f"Expected at least 3 successful webhooks, got {len(successful_webhooks)}"
        )
    ...
    def test_subscription_upgrade_downgrade_flows(self, test_client, mock_user_token):
        ...
        with patch(
            ...
            if enterprise_response.status_code == 200:
                self._simulate_successful_payment(
                    test_client, enterprise_response.json()
                )

            # Test 3: Downgrade scenarios
            downgrade_response = test_client.post(
                "/api/v1/subscriptions/cancel/test_sub_id", headers=headers
            )

            # Should handle downgrade request
            assert downgrade_response.status_code in [200, 404, 401]

    def _simulate_successful_payment(
        self, client: TestClient, auth_data: Dict[str, Any]
    ):
...
class TestErrorHandlingAndRecovery:
    ...
    def test_database_connection_failure_handling(self, test_client):
        ...
        with patch("src.coaching_assistant.core.database.get_db") as mock_db:
            ...
            if response.status_code == 200:
                ...
                assert health_data.get("database") in [
                    "error",
                    "unavailable",
                    None,
                ]
    ...
    def test_ecpay_api_timeout_handling(self, test_client):
        ...
        with patch("requests.post") as mock_request:
            ...
            auth_request = {"plan_id": "PRO", "billing_cycle": "monthly"}
