...
class TestReceiptDownloadE2E:
    ...
    def test_complete_receipt_download_flow(
        ...
        # Step 5: Simulate frontend processing (what handleDownloadReceipt
        # would do)
    ...
    def test_receipt_download_with_different_plans(
        self, test_client: TestClient, test_user: User, test_db: Session
    ):
    ...
    def test_receipt_accessibility_and_format(
        self, test_client: TestClient, test_user: User, test_db: Session
    ):
        ...
        assert (
            "&" not in receipt["customer"]["name"]
        )  # Should not contain HTML entities in data
...
<head>
    ...
    <style>
        ...
        .section h3 {{font-size: 16px; font-weight: bold; border-bottom: 1px solid #ccc;
