...
class LeMURPipelineTester:
    ...
    async def upload_audio(self, session_id: str, audio_file_path: Path) -> str:
        ...
        async with httpx.AsyncClient(timeout=60.0) as client:
            ...
            if filename.lower().endswith((".mp3", ".wav", ".m4a")):
                content_type = (
                    "audio/mpeg" if filename.lower().endswith(".mp3") else "audio/wav"
                )
            ...
            if upload_response.status_code not in [200, 204]:
                raise RuntimeError(
                    f"Failed to upload audio: {upload_response.status_code}"
                )
    ...
    async def start_transcription(self, session_id: str) -> bool:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if response.status_code != 200:
                raise RuntimeError(
                    f"Failed to start transcription: {response.status_code} - {response.text}"
                )
    ...
    async def wait_for_transcription_completion(
        self, session_id: str, timeout_seconds: int = TIMEOUT_SECONDS
    ) -> bool:
        """Wait for transcription to complete."""
        async with httpx.AsyncClient(timeout=30.0) as client:
            logger.info(
                f"⏳ Waiting for transcription completion (timeout: {timeout_seconds}s)"
            )
            ...
            while time.time() - start_time < timeout_seconds:
                ...
                logger.info(
                    f"📊 Transcription progress: {progress}% - Status: {status}"
                )
    ...
    async def apply_lemur_speaker_identification(
        ...
        async with httpx.AsyncClient(timeout=120.0) as client:
            ...
            logger.info(
                f"✅ Speaker identification completed - Speaker mapping: {
                    result.get('speaker_mapping', {})
                }"
            )
            return result

    async def apply_lemur_punctuation_optimization(
        self, session_id: str, custom_prompt: Optional[str] = None
    ) -> Dict:
        ...
        async with httpx.AsyncClient(timeout=120.0) as client:
            ...
            result = response.json()
            logger.info(
                f"✅ Punctuation optimization completed - Improvements: {
                    result.get('improvements_made', [])
                }"
            )
            return result
    ...
    async def get_optimized_transcript(self, session_id: str) -> List[Dict]:
        ...
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if response.status_code != 200:
                raise RuntimeError(
                    f"Failed to get optimized transcript: {response.status_code} - {response.text}"
                )
    ...
    def compare_transcripts(
        self, original_segments: List[Dict], optimized_segments: List[Dict]
    ) -> Dict:
    ...
    async def run_full_pipeline_test(
        ...
        try:
            ...
            speaker_result = await self.apply_lemur_speaker_identification(
                session_id, custom_speaker_prompt
            )
            punctuation_result = await self.apply_lemur_punctuation_optimization(
            ...
            results = {
                ...
                "punctuation_improvements": punctuation_result.get(
                    "improvements_made", []
                ),
