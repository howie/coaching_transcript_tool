...
class TestPlanUpgradeE2E:
    ...
    def test_usage_limit_warnings_and_notifications(self):
        ...
        if response.status_code == 200:
            ...
            assert status.get("warning_level") is None or status.get("warning_level") == "none"
    ...
    def test_failed_payment_handling(self):
        ...
        with patch('stripe.PaymentIntent.create') as mock_payment:
            ...
            error = response.json()
            assert "payment" in error.get("detail", "").lower() or \
    ...
    def test_export_format_restrictions_e2e(self):
        ...
        business_user = self.create_test_user("business_export@test.com", UserPlan.BUSINESS)
        ...
        response = self.client.get(
            "/api/sessions/1/transcript?format=txt",
        ...
        response = self.client.get(
            "/api/sessions/1/transcript?format=xlsx",
            headers=free_headers
        )
    ...
    def test_monthly_usage_reset_e2e(self):
        ...
        response = self.client.post(
            "/api/v1/plan/reset-monthly-usage",
            headers=auth_headers
        )
    ...
    def test_concurrent_processing_limits_e2e(self):
        ...
        response = self.client.post(
            "/api/sessions/2/start-transcription",
            headers=auth_headers
        )
        ...
        response = self.client.post(
            "/api/sessions/4/start-transcription",
            headers=auth_headers
        )
    ...
    async def test_webhook_notifications_e2e(self):
        ...
        with patch('httpx.AsyncClient.post') as mock_webhook:
            ...
            response = self.client.post(
                f"/api/sessions/1/start-transcription",
