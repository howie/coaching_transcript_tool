...
class E2EAuthHelper:
    ...
    def create_test_user(
        self, db: Session, email: str, plan: UserPlan = UserPlan.FREE, **kwargs
    ) -> User:
        """
    ...
    def create_users_with_different_plans(self, db: Session) -> Dict[str, User]:
        ...
        users = {
            "free": self.create_test_user(
                db, "free_user@test.com", UserPlan.FREE, name="Free User"
            ),
            "pro": self.create_test_user(
                db, "pro_user@test.com", UserPlan.PRO, name="Pro User"
            ),
    ...
    def simulate_usage_near_limit(
        self, db: Session, user: User, percentage: float = 0.8
    ):
        """
        ...
        if limits["max_transcription_count"] != -1:  # Not unlimited
            user.transcription_count = int(
                limits["max_transcription_count"] * percentage
            )
