...
class PaymentQATestRunner:
    ...
    def __init__(self):
        self.test_suites = {
            ...
            "regression": {
                ...
                "files": ["tests/regression/test_payment_error_scenarios.py"],
                "description": "Prevent previously fixed bugs from reoccurring",
            },
            "browser": {
                "name": "Browser Compatibility Tests",
                "files": ["tests/compatibility/test_browser_compatibility.py"],
                "description": "Multi-browser and device compatibility testing",
            },
            "monitoring": {
                "name": "Monitoring & Metrics Tests",
                "files": ["tests/monitoring/test_payment_success_monitoring.py"],
    ...
    def run_test_suite(
        self, suite_name: str, verbose: bool = False, coverage: bool = False
    ) -> Dict:
    ...
    def _run_pytest_file(
        self, test_file: str, verbose: bool = False, coverage: bool = False
    ) -> Dict:
        ...
        if importlib.util.find_spec("pytest_json_report") is not None:
        ...
        try:
            ...
            print(
                f"   {status} - {test_stats.get('total', 0)} tests in {duration:.2f}s"
            )

            if result.returncode != 0 and not verbose:
                print("   🔍 Use --verbose for detailed error output")
    ...
    def _run_suites_parallel(self, verbose: bool, coverage: bool) -> Dict:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
            future_to_suite = {
                executor.submit(
                    self.run_test_suite, suite_name, verbose, coverage
                ): suite_name
                for suite_name in self.test_suites.keys()
            }
    ...
    def _print_overall_summary(self, all_results: Dict):
        ...
        total_tests = sum(
            result.get("total_tests", 0) for result in all_results.values()
        )
        ...
        for suite_name, result in all_results.items():
            ...
            print(
                f"   {status} {result.get('name', suite_name):30} ({tests:3d} tests, {duration:6.2f}s)"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Payment System Quality Assurance Test Runner"
    )
    parser.add_argument(
        "--suite",
        choices=["all", "e2e", "regression", "browser", "monitoring", "webhook"],
        default="all",
    ...
    try:
        if args.suite == "all":
            ...
            results = {
                args.suite: runner.run_test_suite(
                    args.suite, args.verbose, args.coverage
                )
            }
