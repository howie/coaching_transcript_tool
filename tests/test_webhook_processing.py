...
class TestWebhookProcessing:
    ...
    def test_authorization_callback_success(
        ...
        with patch.object(
            ECPaySubscriptionService, "_verify_callback", return_value=True
        ):
            response = test_client.post("/api/webhooks/ecpay-auth", data=callback_data)
    ...
    def test_authorization_callback_verification_failed(
        self, db_session: Session, test_client: TestClient
    ):
        ...
        with patch.object(
            ECPaySubscriptionService, "_verify_callback", return_value=False
        ):
            response = test_client.post("/api/webhooks/ecpay-auth", data=callback_data)
    ...
    def test_billing_callback_success(
        self, db_session: Session, test_client: TestClient
    ):
        ...
        with (
            ...
            patch.object(
                ECPaySubscriptionService,
                "_handle_payment_success_notifications",
            ) as mock_success_notif,
        ):
            response = test_client.post(
                "/api/webhooks/ecpay-billing", data=webhook_data
            )
        ...
        payment = (
            ...
            .filter(SubscriptionPayment.gwsr == "TEST_GWSR_123456")
            .first()
        )
