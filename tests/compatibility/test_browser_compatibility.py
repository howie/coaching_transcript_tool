...
class TestBrowserCompatibility:
    ...
    def test_billing_page_loads_cross_browser(self, browser_driver):
        ...
        try:
            ...
            essential_elements = [
                ...
                (
                    By.XPATH,
                    "//*[contains(@class, 'plan') or contains(@class, 'billing')]",
                ),
            ]
            ...
            assert page_title is not None and page_title != "", (
                f"Page title empty in {browser_name}"
            )

        except TimeoutException:
            pytest.fail(
                f"Billing page failed to load in {browser_name} within {TEST_TIMEOUT}s"
            )
    ...
    def test_plan_comparison_display_cross_browser(self, browser_driver):
        ...
        try:
            ...
            assert len(plan_elements) > 0, (
                f"No plan information visible in {browser_name}"
            )

            # Check for pricing information
            price_elements = driver.find_elements(
                By.XPATH,
                "//*[contains(text(), 'NT$') or contains(text(), '$') or "
                "contains(text(), '899') or contains(text(), '2999')]",
            )

            # Should show some pricing info
            assert len(price_elements) > 0 or len(plan_elements) >= 3, (
                f"Pricing not displayed correctly in {browser_name}"
            )
    ...
    def test_upgrade_button_functionality_cross_browser(self, browser_driver):
        ...
        try:
            ...
            upgrade_button_selectors = [
                (
                    By.XPATH,
                    "//button[contains(text(), '升級') or contains(text(), 'Upgrade')]",
                ),
                (
                    By.XPATH,
                    "//a[contains(text(), '升級') or contains(text(), 'Upgrade')]",
                ),
                (By.CLASS_NAME, "upgrade-button"),
                (By.CLASS_NAME, "plan-upgrade"),
                (
                    By.XPATH,
                    "//button[contains(@class, 'upgrade') or contains(@class, 'plan')]",
                ),
            ]
            ...
            if upgrade_button:
                ...
                assert upgrade_button.is_enabled(), (
                    f"Upgrade button not enabled in {browser_name}"
                )
                ...
                assert navigation_occurred or modal_appeared, (
                    f"Upgrade button click had no effect in {browser_name}"
                )
    ...
    def test_mobile_billing_page_responsive(self, mobile_driver):
        ...
        try:
            ...
            assert viewport_width <= 500, (
                f"Mobile viewport not set correctly: {viewport_width}px"
            )
            ...
            assert page_width <= window_width + 20, (
                f"Horizontal scroll detected on {device_name}: page {page_width}px > window {window_width}px"
            )
    ...
    def test_mobile_plan_selection_usability(self, mobile_driver):
        ...
        try:
            ...
            plan_buttons = driver.find_elements(
                By.XPATH,
                "//button[contains(text(), 'PRO') or contains(text(), 'ENTERPRISE')] | "
                "//a[contains(text(), 'PRO') or contains(text(), 'ENTERPRISE')]",
            )
            ...
            if plan_buttons:
                ...
                for button in plan_buttons[:2]:  # Test first 2 buttons
                    ...
                    assert size["height"] >= 30, (
                        f"Button too small for touch on {device_name}: {size['height']}px height"
                    )
                    assert size["width"] >= 60, (
                        f"Button too narrow for touch on {device_name}: {size['width']}px width"
                    )
...
class TestAPICompatibility:
    ...
    def test_api_endpoints_cross_browser(self, user_agent):
        ...
        assert response.status_code == 200, (
            f"Health check failed for user agent: {user_agent}"
        )
        ...
        assert response.status_code in [
            200,
            404,
        ], f"Plans endpoint failed for user agent: {user_agent}"
    ...
    def test_cors_headers_compatibility(self):
        ...
        for origin in test_origins:
            ...
            response = requests.options(
                f"{API_BASE_URL}/api/v1/plans/compare", headers=headers
            )
            ...
            assert response.status_code in [
                ...
                404,
                405,
            ], f"CORS preflight failed for origin: {origin}"
    ...
    def test_content_type_compatibility(self):
        ...
        # Should handle form data correctly (success or expected validation
        # error)
        assert response.status_code in [
            200,
            400,
    ...
    def test_character_encoding_compatibility(self):
        ...
        assert response.status_code in [
            ...
            400,
            405,
        ], "UTF-8 encoding handling failed"
...
class TestJavaScriptCompatibility:
    ...
    def test_browser_javascript_features(self, browser_driver):
        ...
        try:
            ...
            assert js_result == "object", (
                f"localStorage not available in {browser_name}"
            )
...
def run_browser_compatibility_tests():
    ...
    pytest.main(
        [
            ...
            "--tb=short",
            "-k",
            "not test_mobile",  # Skip mobile tests if drivers not available
        ]
    )
