...
def upgrade() -> None:
    """Upgrade schema."""
    # Use existing userplan enum type instead of creating new one
    from sqlalchemy.dialects.postgresql import ENUM

    userplan_enum = ENUM(
        "FREE", "PRO", "ENTERPRISE", name="userplan", create_type=False
    )
    ...
    op.create_table(
        ...
        sa.Column("plan_type", userplan_enum, nullable=False),
    ...
    op.create_index(
        op.f("ix_plan_configurations_plan_type"),
        "plan_configurations",
        ["plan_type"],
        unique=True,
    )
    op.create_table(
        ...
        sa.Column("old_plan", userplan_enum, nullable=True),
        sa.Column("new_plan", userplan_enum, nullable=False),
    ...
    op.create_index(
        op.f("ix_role_audit_log_changed_by_id"),
        "role_audit_log",
        ["changed_by_id"],
        unique=False,
    )
    op.create_index(
        op.f("ix_role_audit_log_user_id"), "role_audit_log", ["user_id"], unique=False
    )
    op.alter_column(
        "session",
        "provider_metadata",
...
def downgrade() -> None:
    ...
    op.create_index(
        op.f("idx_usage_logs_user_created"),
        "usage_logs",
        ["user_id", "created_at"],
        unique=False,
    )
    op.create_index(
        op.f("idx_usage_logs_session"), "usage_logs", ["session_id"], unique=False
    )
    op.alter_column(
        "usage_logs",
        "provider_metadata",
    ...
    op.create_index(
        op.f("idx_usage_analytics_user"),
        "usage_analytics",
        ["user_id", "month_year"],
        unique=False,
    )
    op.create_index(
        op.f("idx_usage_analytics_month"),
        "usage_analytics",
        ["month_year"],
        unique=False,
    )
    ...
    op.create_index(
        op.f("idx_role_audit_user"), "role_audit_log", ["user_id"], unique=False
    )
    op.create_index(
        op.f("idx_role_audit_created_at"),
        "role_audit_log",
        ["created_at"],
        unique=False,
    )
    op.create_index(
        op.f("idx_role_audit_changed_by"),
        "role_audit_log",
        ["changed_by_id"],
        unique=False,
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "period_type", sa.VARCHAR(length=20), autoincrement=False, nullable=False
        ),
        sa.Column(
            "period_start",
            postgresql.TIMESTAMP(timezone=True),
        ...
        sa.Column(
            "plan_name", sa.VARCHAR(length=20), autoincrement=False, nullable=True
        ),
        sa.Column(
            "plan_limits",
        ...
        sa.ForeignKeyConstraint(
            ["user_id"],
            ["user.id"],
            name=op.f("usage_history_user_id_fkey"),
            ondelete="CASCADE",
        ),
        ...
        sa.UniqueConstraint(
            ...
            "period_type",
            "period_start",
            name=op.f("unique_user_period"),
            postgresql_include=[],
            postgresql_nulls_not_distinct=False,
        ),
    )
    op.create_index(
        op.f("idx_usage_history_user_period"),
        "usage_history",
        ["user_id", "period_start"],
        unique=False,
    )
    op.create_index(
        op.f("idx_usage_history_recorded"),
        "usage_history",
        ["recorded_at"],
        unique=False,
    )
    ...
    op.drop_index(
        op.f("ix_plan_configurations_plan_type"), table_name="plan_configurations"
    )
