...
def upgrade() -> None:
    ...
    sessionsource_enum = postgresql.ENUM('CLIENT', 'FRIEND', 'CLASSMATE', 'SUBORDINATE', name='sessionsource')
    sessionsource_enum.create(op.get_bind(), checkfirst=True)
    
    # Add source column to coaching_session table
    op.add_column('coaching_session', sa.Column('source', sessionsource_enum, nullable=False, server_default='CLIENT'))
    
    # Recreate indexes that were dropped in previous migrations
    op.create_index(op.f('idx_sessions_coach_date'), 'coaching_session', ['coach_id', sa.desc('session_date')], unique=False)
