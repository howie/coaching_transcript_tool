...
def upgrade() -> None:
    ...
    role_enum = postgresql.ENUM(
        "user", "staff", "admin", "super_admin", name="userrole"
    )
    ...
    op.add_column(
        ...
        sa.Column(
            "role",
            sa.Enum("user", "staff", "admin", "super_admin", name="userrole"),
            nullable=False,
            server_default="user",
        ),
    )
    ...
    op.add_column(
        "user", sa.Column("last_admin_login", sa.DateTime(timezone=True), nullable=True)
    )
    op.add_column(
        "user",
        sa.Column("admin_access_expires", sa.DateTime(timezone=True), nullable=True),
    )
