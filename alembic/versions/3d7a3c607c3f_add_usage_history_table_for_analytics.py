...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "audio_minutes_processed", sa.DECIMAL(precision=10, scale=2), nullable=False
        ),
        ...
        sa.UniqueConstraint(
            "user_id", "period_type", "period_start", name="uq_user_period_start"
        ),
    )
    op.create_index(
        "idx_usage_history_period_type", "usage_history", ["period_type"], unique=False
    )
    op.create_index(
        "idx_usage_history_recorded", "usage_history", ["recorded_at"], unique=False
    )
    op.create_index(
        "idx_usage_history_user_period",
        "usage_history",
