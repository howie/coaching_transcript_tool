...
def upgrade() -> None:
    ...
    if 'session_count' not in existing_columns:
        op.add_column('user', sa.Column('session_count', sa.Integer(), nullable=True, server_default='0'))
    
    # Add transcription_count if it doesn't exist
    if 'transcription_count' not in existing_columns:
        op.add_column('user', sa.Column('transcription_count', sa.Integer(), nullable=True, server_default='0'))
    
    # Add usage_minutes if it doesn't exist
    if 'usage_minutes' not in existing_columns:
        op.add_column('user', sa.Column('usage_minutes', sa.Integer(), nullable=True, server_default='0'))
    
    # Add last_usage_reset timestamp if it doesn't exist
    if 'last_usage_reset' not in existing_columns:
        op.add_column('user', sa.Column('last_usage_reset', sa.DateTime(), nullable=True))
    ...
    op.create_table('usage_history',
        ...
        sa.UniqueConstraint('user_id', 'period_type', 'period_start', name='unique_user_period')
    )
    
    # Create indexes for performance
    op.create_index('idx_usage_history_user_period', 'usage_history', ['user_id', 'period_start'])
    op.create_index('idx_usage_history_recorded', 'usage_history', ['recorded_at'])
    
    # Create index on user table for usage queries
    op.create_index('idx_user_usage_tracking', 'user', ['session_count', 'transcription_count', 'usage_minutes'])
