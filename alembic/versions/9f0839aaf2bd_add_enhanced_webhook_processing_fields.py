...
def upgrade() -> None:
    ...
    op.add_column('saas_subscriptions', sa.Column('grace_period_ends_at', sa.DateTime(timezone=True), nullable=True))
    op.add_column('saas_subscriptions', sa.Column('downgraded_at', sa.DateTime(timezone=True), nullable=True))
    op.add_column('saas_subscriptions', sa.Column('downgrade_reason', sa.String(50), nullable=True))
    
    # Add retry fields to subscription_payments
    op.add_column('subscription_payments', sa.Column('next_retry_at', sa.DateTime(timezone=True), nullable=True))
    op.add_column('subscription_payments', sa.Column('max_retries', sa.Integer, nullable=True, default=3))
    op.add_column('subscription_payments', sa.Column('last_retry_at', sa.DateTime(timezone=True), nullable=True))
    
    # Add index for retry processing
    op.create_index('ix_subscription_payments_retry_at', 'subscription_payments', ['next_retry_at'])
    
    # Add index for grace period processing
    op.create_index('ix_saas_subscriptions_grace_period', 'saas_subscriptions', ['grace_period_ends_at'])
...
def downgrade() -> None:
    ...
    op.drop_index('ix_subscription_payments_retry_at', table_name='subscription_payments')
