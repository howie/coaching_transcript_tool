...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('status', sa.String(20), default='received', nullable=False, index=True),
        ...
        sa.Column('subscription_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('saas_subscriptions.id'), nullable=True, index=True),
        sa.Column('payment_id', postgresql.UUID(as_uuid=True), sa.ForeignKey('subscription_payments.id'), nullable=True, index=True),
    ...
    op.create_index('idx_webhook_logs_type_status', 'webhook_logs', ['webhook_type', 'status'])
    op.create_index('idx_webhook_logs_merchant_member', 'webhook_logs', ['merchant_member_id'])
