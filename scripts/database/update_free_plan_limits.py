...
def update_free_plan_limits():
    ...
    with engine.connect() as conn:
        ...
        result = conn.execute(
            text("SELECT id, limits FROM plan_configurations WHERE plan_name = 'free'")
        )
        ...
        current_limits = (
            free_plan.limits
            if isinstance(free_plan.limits, dict)
            else json.loads(free_plan.limits)
        )
        ...
        """),
            {"limits": json.dumps(new_limits), "updated_at": datetime.now(UTC)},
        )
        ...
        # Verify update
        result = conn.execute(
            text("SELECT limits FROM plan_configurations WHERE plan_name = 'free'")
        )
        updated_plan = result.fetchone()
        updated_limits = (
            updated_plan.limits
            if isinstance(updated_plan.limits, dict)
            else json.loads(updated_plan.limits)
        )
        ...
        print("\n🔄 Changes made:")
        print(
            f"  - Sessions: {current_limits['maxSessions']} → {updated_limits['maxSessions']}"
        )
        print(
            f"  - Minutes: {current_limits['maxMinutes']} → {updated_limits['maxMinutes']}"
        )
        print(
            f"  - File size: {current_limits['maxFileSize']}MB → {updated_limits['maxFileSize']}MB"
        )
