...
from coaching_assistant.services.plan_configuration_service import PlanConfigurationService
...
class PlanConfigurationValidator:
    ...
    def validate_database_configuration(self) -> bool:
        ...
        try:
            ...
            for plan in plan_configs:
                ...
                if (plan.monthly_price_twd_cents != monthly_expected or 
                    plan.annual_price_twd_cents != annual_expected):
                    self.add_result(
        ...
        try:
            ...
            for user_plan, expected_display_name in test_cases:
                try:
                    ...
                    # Validate structure
                    required_keys = ['plan_type', 'display_name', 'limits', 'monthly_price_twd_cents']
                    ...
                    if limits.get('max_sessions') != -1 or limits.get('max_transcription_count') != -1:
                        self.add_result(
                            f"Service - {user_plan.value} Phase 2 Limits",
                            False,
                            f"Sessions/Transcriptions should be unlimited (-1)"
                        )
                        continue
    ...
    def validate_plan_limits_service(self) -> bool:
        ...
        try:
            ...
            test_plans = [PlanName.FREE, PlanName.STUDENT, PlanName.PRO, PlanName.COACHING_SCHOOL]
            ...
            for plan_name in test_plans:
                try:
                    ...
                    if plan_limit.max_sessions != -1 or plan_limit.max_transcriptions != -1:
                        self.add_result(
...
async def main():
    ...
    try:
        with PlanConfigurationValidator(verbose=verbose) as validator:
            ...
            results_file = Path("tmp") / f"validation_results_{summary['passed_tests']}_{summary['total_tests']}.json"
