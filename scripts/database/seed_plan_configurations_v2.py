#!/usr/bin/env python3
"""
Phase 2 seed script for plan configurations.
Updated to include Phase 2 requirements:
- Only minutes-based limits (sessions and transcriptions unlimited)
- Updated plan names and pricing
- New STUDENT plan (學習方案)
- ENTERPRISE renamed to COACHING_SCHOOL (認證學校方案)
...
def get_phase2_plan_configurations() -> List[Dict[str, Any]]:
...
def seed_plan_configurations_v2(db: Session):
    """Seed Phase 2 plan configurations."""

    logger.info("🌱 Starting Phase 2 plan configuration seeding...")

    # Check existing plans
    existing_count = db.query(PlanConfiguration).count()
    ...
    try:
        db.commit()
        logger.info(
            f"✅ Successfully seeded {inserted_count} Phase 2 plan configurations"
        )
    except Exception as e:
        db.rollback()
...
def verify_phase2_seed(db: Session):
    ...
    actual_plans = [p.plan_name for p in plans]

    if actual_plans != expected_plans:
        logger.error(
            f"❌ Plan name mismatch. Expected: {expected_plans}, Got: {actual_plans}"
        )
        return False
    ...
    for plan in plans:
        limits = plan.limits
        pricing_twd = (
            plan.monthly_price_twd_cents / 100 if plan.monthly_price_twd_cents else 0
        )
        ...
        compliance_status = (
            "✅ Phase 2 Compliant" if phase2_compliant else "❌ Not Phase 2 Compliant"
        )

        logger.info(f"\n  🏷️  {plan.display_name} ({plan.plan_name}):")
        logger.info(f"    💰 Price: NT${pricing_twd:.0f}/月")
        logger.info(
            f"    🎯 Sessions: {'無限制' if sessions_limit == -1 else sessions_limit}"
        )
        logger.info(
            f"    ⏱️  Minutes: {'無限制' if minutes_limit == -1 else f'{minutes_limit} 分鐘/月'}"
        )
        logger.info(
            f"    📝 Transcriptions: {'無限制' if transcriptions_limit == -1 else transcriptions_limit}"
        )
        ...
        if plan.plan_name == "coaching_school":
            ...
            extra_student_price = (
                extra_data.get("additional_student_price_twd_cents", 0) / 100
            )
...
def main():
    ...
    try:
        ...
        seed_plan_configurations_v2(db)
        ...
        if verify_phase2_seed(db):
            ...
            logger.info(
                "  ✅ 認證學校方案 (Coaching School Plan) - 無限制, NT$5000/月起"
            )
            logger.info("\n🔥 Phase 2 Features:")
