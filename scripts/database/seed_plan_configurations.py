...
def seed_plan_configurations(db: Session):
    ...
    if existing_plans > 0:
        logger.info(
            f"Found {existing_plans} existing plan configurations. Skipping seed."
        )
        return
    ...
    free_plan = PlanConfiguration(
        ...
        is_active=True,
        is_visible=True,
        extra_data={"stripe_product_id": None, "trial_days": 0, "max_team_members": 1},
    )
...
def verify_seed(db: Session):
    ...
    for plan in plans:
        ...
        logger.info(
            f"    - Monthly Price: ${plan.monthly_price_cents / 100:.2f} USD / NT${plan.monthly_price_twd_cents / 100:.0f} TWD"
        )
        logger.info(
            f"    - Annual Price: ${plan.annual_price_cents / 100:.2f} USD / NT${plan.annual_price_twd_cents / 100:.0f} TWD"
        )
        logger.info(
            f"    - Max Sessions: {plan.limits['max_sessions'] if plan.limits['max_sessions'] != -1 else 'Unlimited'}"
        )
        logger.info(
            f"    - Max Minutes: {plan.limits['max_total_minutes'] if plan.limits['max_total_minutes'] != -1 else 'Unlimited'}"
        )
