...
def seed_beta_plan_configs():
    ...
    plan_configs = [
        {
            ...
            "is_active": True,
            "is_visible": True,
            "extra_data": {"beta_limits": True, "cost_control": "strict"},
        },
        {
            "id": str(uuid.uuid4()),
            "plan_type": "PRO",
            ...
            "is_active": True,
            "is_visible": True,
            "extra_data": {"beta_limits": True, "cost_control": "moderate"},
        },
        {
            "id": str(uuid.uuid4()),
    ...
    with engine.connect() as conn:
        ...
        if count > 0:
            print(
                f"⚠️  Plan configurations already exist ({count} records). Skipping seed."
            )
            return
        ...
        result = conn.execute(
            text(
                "SELECT plan_name, display_name FROM plan_configurations ORDER BY sort_order"
            )
        )
        plans = list(result)
        ...
        result = conn.execute(
            text(
                "SELECT plan_name, limits FROM plan_configurations WHERE plan_name = 'free'"
            )
        )
        free_plan = result.fetchone()
        if free_plan:
            print("\n📊 Free plan limits:")
            print("  - Sessions: 10 per month")
            print("  - Transcription minutes: 200 per month")
            print("  - Transcriptions: 5 per month")
            print("  - Max file size: 60MB (up to 40 min per recording)")
