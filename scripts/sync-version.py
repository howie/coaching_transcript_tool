...
def update_package_json(file_path: Path, version: str) -> bool:
    ...
    if package_data.get("version") != version:
        ...
        print(
            f"{GREEN}✓ Updated {file_path.relative_to(file_path.parent.parent.parent)} to version {version}{RESET}"
        )
        return True
    else:
        print(
            f"{BLUE}ℹ {file_path.relative_to(file_path.parent.parent.parent)} already at version {version}{RESET}"
        )
        return False
    ...
    if pyproject_file.exists():
        ...
        if match and match.group(1) != master_version:
            inconsistencies.append(
                f"pyproject.toml: {match.group(1)} (expected {master_version})"
            )
    ...
    for package_file in find_package_json_files(root_dir):
        with open(package_file, "r", encoding="utf-8") as f:
            ...
            if (
                ...
                and package_data["version"] != master_version
            ):
                rel_path = package_file.relative_to(root_dir)
                inconsistencies.append(
                    f"{rel_path}: {package_data['version']} (expected {master_version})"
                )
...
def main():
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "--check":
            ...
            print("\nOptions:")
            print(
                "  --check    Only check for version consistency (exit 1 if inconsistent)"
            )
