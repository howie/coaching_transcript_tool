...
def create_test_user(
    ...
    if existing_user:
        print(
            f"✅ Test user already exists: {existing_user.email} (ID: {existing_user.id})"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Setup test authentication for payment system tests"
    )
    parser.add_argument(
        "--create-test-user", action="store_true", help="Create a test user in database"
    )
    parser.add_argument(
        "--generate-token", action="store_true", help="Generate JWT tokens"
    )
    parser.add_argument("--user-id", help="User ID to generate tokens for")
    parser.add_argument(
        "--export-env", action="store_true", help="Output environment variable exports"
    )
    ...
    if args.generate_token or args.create_test_user:
        ...
        if args.export_env or not args.generate_token:
            ...
            print(
                f'curl -H "Authorization: Bearer {tokens["access_token"][:50]}..." \\'
            )
            print("     http://localhost:8000/api/v1/auth/me")

    print("\n✅ Authentication setup complete!")
    print("🔥 You can now run authenticated payment tests:")
    print("   python tests/run_payment_qa_tests.py --suite e2e")
