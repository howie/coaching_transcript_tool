...
def check_enum_data_consistency():
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            valid_values = ['FREE', 'PRO', 'ENTERPRISE', 'free', 'pro', 'enterprise', 'student', 'coaching_school']
            ...
            for table_name, column_name in tables_to_check:
                try:
                    ...
                    if invalid_values:
                        print(f"   ⚠️  Invalid values in {table_name}.{column_name}: {invalid_values}")
                    else:
                        print(f"   ✅ All values valid in {table_name}.{column_name}")

                except Exception as e:
                    print(f"   ❌ Error checking invalid values in {table_name}.{column_name}: {e}")
