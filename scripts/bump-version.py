...
from sync_version import BLUE, GREEN, RED, RESET, YELLOW, sync_all_versions
...
def main():
    ...
    if len(sys.argv) < 2:
        print(
            f"{RED}Usage: python bump-version.py <major|minor|patch> [description]{RESET}"
        )
        sys.exit(1)
    ...
    if sync_all_versions(root_dir):
        ...
        print("  1. Update release notes in version.json")
        print("  2. Update CHANGELOG.md with detailed changes")
        print("  3. Commit the changes:")
        print("     git add -A")
        print(f'     git commit -m "chore: bump version to {new_version}"')
        print("  4. Create a tag (after merging to main):")
