...
def check_environment():
    ...
    required_vars = ["DATABASE_URL", "REDIS_URL", "SMTP_USER", "SMTP_PASSWORD"]

    optional_vars = ["SMTP_SERVER", "SMTP_PORT", "SENDER_EMAIL", "ADMIN_REPORT_EMAILS"]
    ...
    if missing_required:
        ...
        print()
        print(
            "Please configure these variables before running the admin report system."
        )
        return False
...
def generate_test_report(target_date: str = None):
    ...
    try:
        ...
        with get_db_session() as db:
            ...
            print(
                f"   - Minutes processed: {float(report_data.total_minutes_processed):.1f}"
            )
...
def test_email_configuration():
    ...
    try:
        ...
        msg = MimeText(
            "This is a test email from Admin Report Setup. ✅", "plain", "utf-8"
        )
...
def main():
    ...
    parser.add_argument(
        "--test-db", action="store_true", help="Test database connection"
    )
    parser.add_argument(
        "--test-email", action="store_true", help="Test email configuration"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--target-date", type=str, help="Target date for test report (YYYY-MM-DD)"
    )
    ...
    if success:
        ...
        print(
            "❌ Some tests failed. Please fix the issues before using the admin report system."
        )
        sys.exit(1)
