...
def reset_stuck_sessions(timeout_minutes=30, dry_run=True):
    ...
    """
    logger.info(f"Starting session reset check (timeout: {timeout_minutes} minutes, dry_run: {dry_run})")
    ...
    try:
        ...
        # Find stuck sessions
        stuck_sessions = db.query(Session).filter(
        ...
        if not dry_run:
            # Commit all changes
            db.commit()
            logger.info(f"✅ Successfully committed changes - reset {len(stuck_sessions)} sessions")
        else:
            logger.info(f"🔍 DRY RUN complete - found {len(stuck_sessions)} sessions that would be reset")
...
def verify_reset_results():
    ...
    try:
        ...
        processing_count = db.query(Session).filter(
            Session.status == SessionStatus.PROCESSING
        ).count()
        ...
        if processing_count == 0:
            ...
            logger.warning(f"⚠️  Warning: {processing_count} sessions still in processing status")
        ...
        logger.info(f"Session Status Summary:")
...
Examples:
  ...
  python scripts/emergency_session_reset.py --timeout 360 --dry-run
    ...
    if not dry_run:
        ...
        print(f"Sessions older than {args.timeout} minutes will be reset to 'failed' status.")
