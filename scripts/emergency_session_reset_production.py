...
def reset_stuck_sessions_sql(timeout_minutes=30, dry_run=True):
    ...
    try:
        ...
        for i, session in enumerate(stuck_sessions, 1):
            ...
            logger.info(
                f"   ⏳ Stuck for: {session.minutes_stuck:.0f} minutes ({hours_stuck:.1f} hours / {days_stuck:.1f} days)"
            )
        ...
        if not dry_run:
            ...
            result = db.execute(
                update_query,
                {"error_message": error_message, "cutoff_time": cutoff_time},
            )
            ...
            if rows_affected != len(stuck_sessions):
                logger.warning(
                    f"⚠️  Warning: Expected to reset {len(stuck_sessions)} sessions but actually reset {rows_affected}"
                )
        else:
            logger.info("")
            logger.info(f"🔍 DRY RUN: Would reset {len(stuck_sessions)} sessions")
            logger.info("💡 Use --execute to actually reset these sessions")
    ...
    try:
        ...
        if remaining_stuck:
            ...
            for session in remaining_stuck:
                logger.warning(
                    f"   🚨 Session {session.id}: {session.minutes_stuck:.0f} minutes"
                )
        else:
...
def list_all_processing_sessions():
    ...
    try:
        ...
        logger.info(
            f"🔍 Found {len(processing_sessions)} sessions in processing status:"
        )
        ...
        for i, session in enumerate(processing_sessions, 1):
            ...
            logger.info(f"   🕐 Updated: {session.updated_at}")
            logger.info(
                f"   ⏳ Processing: {session.minutes_processing:.0f} min ({hours:.1f}h / {days:.1f}d)"
            )
...
Examples:
  ...
  PRODUCTION_DATABASE_URL="..." python scripts/emergency_session_reset_production.py --verify
    ...
    parser.add_argument(
        "--verify", action="store_true", help="Verify no sessions are stuck"
    )

    parser.add_argument(
        "--list", action="store_true", help="List all current processing sessions"
    )
    ...
    if dry_run:
        ...
        if count > 0:
            ...
            print("⚠️  Make sure to backup the database first!")
    else:
