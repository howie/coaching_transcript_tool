...
from coaching_assistant.api.sessions import (
    SessionCreate,
    create_session,
    get_upload_url,
    start_transcription,
...
async def test_session_limit_enforcement():
    ...
    try:
        ...
        print(
            f"👤 Test user: {user.email}, Plan: {user.plan}, Sessions: {user.session_count}"
        )

        # Try to create a session (should fail)
        session_data = SessionCreate(
            title="Test Beta Limit Session", language="en-US", stt_provider="google"
        )
...
async def test_transcription_limit_enforcement():
    ...
    try:
        ...
        if not session:
            ...
            session = (
                db.query(Session).filter(Session.id == session_response.id).first()
            )
        ...
        print(
            f"👤 Test user: {user.email}, Plan: {user.plan}, Transcriptions: {user.transcription_count}"
        )
...
def test_plan_limits_configuration():
    ...
    expected_free = {"sessions": 3, "transcriptions": 5, "file_size": 25}
    ...
    if actual_free == expected_free:
        ...
        print(
            f"❌ FREE plan limits mismatch: expected {expected_free}, got {actual_free}"
        )
        success = False

    # Test PRO plan beta limits
    pro_limits = PlanLimits.from_user_plan("PRO")
    expected_pro = {"sessions": 25, "transcriptions": 50, "file_size": 100}
