...
from coaching_assistant.models.session import Session as TranscriptSession, SessionStatus
...
class DatabaseQueryAgent:
    ...
    def get_system_overview(self) -> Dict[str, Any]:
        ...
        try:
            with get_db_session() as db:
                ...
                enterprise_users = db.query(User).filter(User.plan == UserPlan.ENTERPRISE).count()
                
                # 會話統計
                total_sessions_today = db.query(TranscriptSession).filter(
                    TranscriptSession.created_at >= today_start
                ).count()
                
                completed_sessions = db.query(TranscriptSession).filter(
                    TranscriptSession.created_at >= today_start,
                    TranscriptSession.status == SessionStatus.COMPLETED
                ).count()
                
                failed_sessions = db.query(TranscriptSession).filter(
                    TranscriptSession.created_at >= today_start,
                    TranscriptSession.status == SessionStatus.FAILED
                ).count()
                
                processing_sessions = db.query(TranscriptSession).filter(
                    TranscriptSession.status == SessionStatus.PROCESSING
                ).count()
                
                # 計算成功率
                success_rate = (completed_sessions / total_sessions_today * 100) if total_sessions_today > 0 else 100
                ...
                return [
                    {
                        ...
                        "auth_provider": user.auth_provider if hasattr(user, 'auth_provider') else "email"
                    }
    ...
    def get_recent_sessions(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            with get_db_session() as db:
                recent_sessions = db.query(TranscriptSession).order_by(
                ...
                return [
                    {
                        ...
                        "duration_minutes": round(session.duration_minutes, 1) if session.duration_seconds else 0,
    ...
    def get_user_growth_trend(self, days: int = 7) -> Dict[str, Any]:
        ...
        try:
            with get_db_session() as db:
                ...
                growth_rate = (new_users_period / (total_users - new_users_period) * 100) if (total_users - new_users_period) > 0 else 0
    ...
    def check_system_issues(self) -> Dict[str, Any]:
        ...
        try:
            with get_db_session() as db:
                ...
                # 檢查處理時間過長的會話
                long_processing = db.query(TranscriptSession).filter(
                    TranscriptSession.status == SessionStatus.PROCESSING,
                ...
                # 檢查今日失敗率
                total_today = db.query(TranscriptSession).filter(
                    TranscriptSession.created_at >= today_start
                ).count()
                
                failed_today = db.query(TranscriptSession).filter(
                    TranscriptSession.created_at >= today_start,
                    TranscriptSession.status == SessionStatus.FAILED
                ).count()
                ...
                return {
                    "timestamp": now.isoformat(),
                    "status": "healthy" if not issues else "warning" if not issues and warnings else "issue",
                    "issues": issues,
                    "warnings": warnings,
...
"""
        ...
        if sessions.get('processing', 0) > 5:
            report += f"• 🔄 當前有 {sessions.get('processing', 0)} 個會話在處理，系統較忙\n"
        elif sessions.get('processing', 0) > 0:
            report += f"• 🔄 當前有 {sessions.get('processing', 0)} 個會話在處理，正常\n"
        else:
            report += "• 😴 目前沒有會話在處理\n"
...
def query_recent_activity() -> str:
    ...
    for session in recent_sessions[:3]:
        if "error" not in session:
            report += f"• {session['title']} ({session['status']}, {session['created_at']})\n"
