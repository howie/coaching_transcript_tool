...
def check_daily_usage():
    ...
    with engine.connect() as conn:
        ...
        result = conn.execute(text('SELECT plan, COUNT(*) as count FROM "user" GROUP BY plan'))
        ...
        for stat in usage_stats:
            ...
            print(f"  - {plan}:")
            print(f"    • Avg: {avg_s:.1f} sessions, {avg_t:.1f} transcriptions, {avg_m:.1f} min")
            print(f"    • Max: {max_s} sessions, {max_t} transcriptions, {max_m:.1f} min")
        ...
        if approaching_limits:
            for email, plan, sessions, transcriptions, minutes in approaching_limits:
                print(f"  - {email} ({plan}): {sessions}s, {transcriptions}t, {minutes:.0f}m")
        else:
        ...
        estimated_cost_today = total_transcriptions_today * 0.10  # Rough $0.10 per transcription
...
def main():
    ...
    try:
        ...
        print(f"Next check recommended in 4 hours")
